...
/// @ref stapl::promise compare-and-swap (CAS) latency.
///
/// The main idea of the benchmark is to do a CAS on the remote side and return
/// the result of that CAS.
///
/// It can be compared against
/// -# @c mpi/one-sided/osu_cas_latency.c
/// in OSU benchmarks.
///
/// @note This benchmark is currently only defined for a <tt>long long</tt>.
//////////////////////////////////////////////////////////////////////

#define BENCHMARK "# %s compare-and-swap Benchmark\n"
...
exit_code stapl_main(int argc, char *argv[])
{
  ...
  const value_type v = d_obj.get_value();
    ...
    switch (pr) {
      case rmi_primitive::SYNC_RMI:
        for (int i = 0; i < skip + loop; i++) {
          ...
          sync_rmi(1, r, &dist_T::compare_and_swap, v, v);
        }
        break;
      case rmi_primitive::OPAQUE_RMI:
        for (int i = 0; i < skip + loop; i++) {
          ...
          future<dist_T::proxy_type> f =
            opaque_rmi(1, r, &dist_T::compare_and_swap, v, v);
          f.get();
        }
        break;
      case rmi_primitive::PROMISE_WITH_ASYNC_RMI:
        for (int i = 0; i < skip + loop; i++) {
          ...
          promise<dist_T::proxy_type> p;
          auto f = p.get_future();
          async_rmi(1, r, &dist_T::compare_and_swap_promise,
                    std::move(p), v, v);
          f.get();
        }
        break;
