...
stapl::exit_code stapl_main(int argc,char** argv)
{
  ...
  stapl::map_func(set_coordinates_wf(nx, ny, false), vw);
  stapl::graph_paradigm(set_edges_wf(), set_edges_update_func(), vw);
  // stapl::map_func(init_edges_2D_wf(nx, ny), vw);

  one_print("Testing SWEEP...\t\t\t\t");

  // directions:
  std::vector<dimensions_t> directions =
    {{1,1}, {1,-1}, {-1,1}, {-1,-1}};
  ...
  bool result = stapl::map_reduce(check_level_2D_wf(nx, ny, directions),
