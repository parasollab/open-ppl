...
struct hierarchy_map_wf
{
  ...
  void operator()(NativeGraph&& g, NativeArray&& a)
  {
    ...
    auto leader2 = (*(g.begin() + g.size()/2)).descriptor();
    size_t i=0;
    for (auto e : a) {
      if (i++ < g.size()/2)
        e = leader;
...
stapl::exit_code stapl_main(int argc,char** argv)
{
  ...
  one_print("Creating Hierarchy2 (Setup)\n");
  array_t group_array2(vw1.size());
  array_view_t group_array_vw2(group_array2);

  map_func(hierarchy_map_wf2(), native_view(vw1), native_view(group_array_vw2));

  graph_external_property_map<typename graph_view_t::view_container_type,
                              size_t, array_view_t>
    group_id_prop_map2(vw1.container(), group_array_vw2);

  one_print("Creating Hierarchy2\n");
  graph_view_t vw2 = create_level(vw1, group_id_prop_map2,
                                  vpselect_wf(), epselect_wf());
  rmi_fence();
  ...
  std::vector<graph_view_t*> hierarchy{&vw, &vw1, &vw2};
