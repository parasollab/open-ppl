...
char const* uts_geoshapes_str[] = { "Linear decrease", "Exponential decrease",
...
void uts_parseParams(int argc, char *argv[], problem_params_t& problem_params){
  int i = 1;
  int err = -1;
  while (i < argc && err == -1) {
    ...
    if (!impl_parseParam(argv[i], argv[i+1], problem_params)) {
      i += 2;
      continue;
    }
