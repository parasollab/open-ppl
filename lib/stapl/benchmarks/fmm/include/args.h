...
#ifndef STAPL_BENCHMARKS_FMM_ARGS_H
#define STAPL_BENCHMARKS_FMM_ARGS_H
...
private:
  void usage(char * name)
  {
    fprintf(stderr,
            ...
            " --alltoall (-a)               : All to all pattern (%d)\n"
            " --allgather (-b)              : All gather pattern (%d)\n"
            " --allreduce (-u)              : All scatter pattern (%d)\n"
...
public:
  Args(int argc=0, char ** argv=NULL) : numBodies(1000000), ncrit(16),
       nspawn(1000), threads(16), images(0),
       theta(.4), useRmax(1), useRopt(1), mutual(1), graft(1),
       verbose(1), distribution("cube"), repeat(0),
       alltoall(0),allgather(0),allreduce(0)
  {
    while (1) {
      int option_index;
      int c = getopt_long(argc, argv, "n:c:s:T:i:t:x:o:m:g:v:d:r:a:b:u:h",
      ...
      switch (c)
      {
        ...
        case 'a':
          ...
          break;
        case 'b':
          allgather = atoi(optarg);
          break;
        case 'u':
          allreduce = atoi(optarg);
          break;
        case 'h':
          usage(argv[0]);
          exit(0);
  ...
  void print(int stringLength, int P)
  {
    if (verbose) {                                 // If verbose flag is true
      ...
      << "allgather" << " : " << allgather << "\n"    //  Print distribution
      << std::setw(stringLength)                      //  Set format
      << "allreduce" << " : " << allreduce << std::endl;//  Print distribution
    }                                                // End if for verbose flag
  }
};

#endif // STAPL_BENCHMARKS_FMM_ARGS_H
