...
//////////////////////////////////////////////////////////////////////
/// @file
/// An implementation of http://www.mcs.anl.gov/research/projects/mpi/tutorial/mpiexmpl/src/jacobi/C/main.html
/// using STAPL-RTS zero-copy capabilities.
//////////////////////////////////////////////////////////////////////
...
public:
  ...
  double iteration(void)
  {
    ...
    if (this->get_location_id() < (this->get_num_locations() - 1)) {
      ...
      auto r = make_immutable_range_n(xlocal[m_maxn/this->get_num_locations()],
                                      m_maxn);
    ...
    if (this->get_location_id() > 0) {
      const unsigned int dest = (this->get_location_id() - 1);
      auto r = make_immutable_range_n(xlocal[1], m_maxn);
      async_rmi(dest, this->get_rmi_handle(),
    ...
    const double result = f.get();

    // you can update now that the immutable_range has been consumed
    for (std::size_t i = i_first; i <= i_last; ++i) {
      for (std::size_t j=1; j<m_maxn-1; ++j) {
        xlocal[i][j] = xnew[i][j];
      }
...
exit_code stapl_main(int argc, char* argv[])
{
  ...
  if (rank==0)
    std::cout << "jacobi_1d_armi_zero_copy "
              << get_num_processes() << ' '
