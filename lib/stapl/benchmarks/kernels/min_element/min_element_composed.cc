...
struct outer_init_wf
{
  ...
  void operator()(View vw)
  { iota(vw, 1); }
};


struct print_inner_wf
{
  typedef void result_type;
...
: private Op
{
  ...
  {
    return stapl::reduce(std::forward<V>(v), static_cast<Op const&>(*this));
  }
...
public:
  ...
  : runtime_base_profiler<>("min_element_composed", argc, argv),
    m_ivw(ivw)
  { }
...
exit_code stapl_main(int argc, char* argv[])
{
  ...
  typedef array<value_type>                inner_container_type;
  ...
  outer_container_type c(sizes_vw);
  outer_view_type vw(c);

  // generate data
  map_func(outer_init_wf{}, vw);
