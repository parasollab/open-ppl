...
///          1. The parent's distance is less than the child's
...
public:
  ...
  {
    bool correct_level =
      v.property().level() < m_level;

    bool both_discovered =
      v.property().level() > 0 && m_level > 0;

    // If both vertices were discovered, but parent's distance was more than
    // or equal to child's distance, then the traversal is incorrect
...
///        satsify the following condition:
///          1. Either both vertices were visited or neither were visited
//////////////////////////////////////////////////////////////////////
...
public:
  ...
  {
    desc.add_options()
      ("tau", po::value<double>()->required(),
       "Amount of error allowed upon visitation");
  }

  void parse(po::variables_map const& vars)
  {
    m_tau = vars["tau"].template as<double>();
  }
...
{
  command_line_app<approx_bfs_options> app;
  app(argc, argv);

  const auto k = app.k();
  const auto tau = app.tau();
  const auto trials = app.trials();

  app.supported_paradigms("kla");
  auto vw = app.create_graph<stapl::properties::approximate_bfs_property>();
  ...
  auto bench = benchmark_builder{}
    ...
    .with_runner([&](int trial, sources_type const& sources) {
      return approximate_breadth_first_search(vw ,sources[trial], k, tau);
    })
    .with_stats([&](std::size_t iters) { return iters; })
    .with_verifier([&](std::size_t iters) {
      return verify_approx_bfs(vw);
    })
    .with_trials(trials)
    .build();
