...
int main(int argc, char* argv[]) {
  ...
  if (argc >= 3) ef = atoi(argv[2]);

  MPI_Comm_size(MPI_COMM_WORLD, &size);
  MPI_Comm_rank(MPI_COMM_WORLD, &rank);

  if (rank == 0) fprintf(stderr, "Graph size is %lu vertices and %lu edges\n", (uint64_t)pow(2., log_numverts), (uint64_t)(pow(2., log_numverts) * 8.));
  ...
  make_graph(log_numverts, ef * pow(2., log_numverts), 2, 3, initiator,
             &nedges, &result);
  MPI_Barrier(MPI_COMM_WORLD);
  stop = MPI_Wtime();
