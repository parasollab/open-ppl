...
//////////////////////////////////////////////////////////////////////
/// @file
/// Benchmark suite for Connected Components algorithm on the Graph500 benchmark
/// input (graph500.org).
///
...
///
/// Finally, Connected Components is called on the graph and reports
/// the throughput in MTEPS (Mega Traversed Edges per Second).
///
/// The algorithm can alternatively be run using a hierarchical or hubs CC
...
struct cc_helper
{
  typedef stapl::multidigraph<stapl::properties::cc_property>  graph_t;

  template<typename Graph>
  size_t operator()(Graph& g)
  {
    stapl::do_once([] { std::cout << "starting CC " << std::endl; });

    return stapl::connected_components(g);
  }
...
struct cc_h_helper
{
  ...
  size_t operator()(Graph& g, HGraph& h)
  {
    ...
    return stapl::connected_components(policy, g);
  }

  std::string name(void)
  { return "cc_hierarchical"; }
};


struct cc_hubs_helper
{
  typedef stapl::multidigraph<stapl::properties::cc_property>  graph_t;
...
stapl::exit_code stapl_main(int argc, char* argv[])
{
  ...
  auto input_graph
    = graph500_benchmark_generator<cc_helper>(nv, ef, SCALE);
  size_t nedges = input_graph.num_edges_collective();

  // call the CC experiments.
  if (option == "hierarchical") {
    auto h = create_level_machine_helper(input_graph);
    run_algo(NEXP, nedges, cc_h_helper(), input_graph, h);
  } else if (option == "hubs") {
    auto hubs = create_level_hubs_helper(input_graph, hub_sz);
    auto h = create_level_machine_helper(input_graph);
    run_algo(NEXP, nedges, cc_hubs_helper(), input_graph, h, hubs);
  } else {
    run_algo(NEXP, nedges, cc_helper(), input_graph);
  }
