...
Kernel_3d_ZDG::Kernel_3d_ZDG(Grid_Data* grid)
  : grid_data(grid)
{ }
...
void Kernel_3d_ZDG::LTimes(Grid_Data *grid_data) {
  ...
  // Compute L
  stapl::map_func(zdg_ltimes_wf(grid_data), grid_view);
}
...
public:
  ...
  zdg_lplustimes_wf(Grid_Data* grid)
    : m_grid(grid)
  { }
        ...
        // parallel loop over zones started here
          for (int d = 0; d < num_local_directions; d++) {
            ...
            for(int nm_offset = 0;nm_offset < nidx;++nm_offset){
              ...
              double ell_plus_d_n_m =
                v.property().ell_plus()(ell_plus_idx);
...
void Kernel_3d_ZDG::LPlusTimes(Grid_Data *grid_data) {
  stapl::graph_view<Grid_Data> grid_view(*grid_data);
  stapl::map_func(zdg_lplustimes_wf(grid_data), grid_view);
}
