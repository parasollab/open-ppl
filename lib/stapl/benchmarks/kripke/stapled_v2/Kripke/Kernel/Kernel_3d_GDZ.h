...
{
public:
  static constexpr Nesting_Order nesting_val = NEST_GDZ;
...
struct GDZ_filter
{
  ...
  template <typename V>
  typename result<GDZ_filter(V)>::type
  operator()(V&& v) const
  {
    using result_t = typename result<GDZ_filter(V)>::type;
    result_t res;
    res.resize(v.size());

    for (size_t g = 0; g < v.size() ; ++g)
    {
      res[g].resize(v[g].size());
        for (size_t d = 0; d < v[g].size() ; ++d)
          res[g][d][(size_t)m_direction] =
            std::move(v[g][d][(size_t)m_direction]);
    }
...
template <>
struct kernel_skeleton<NEST_GDZ>
{
  ...
  STAPL_AUTO_RETURN((
    wavefront<num_inputs>(
      ...
      corner, skeleton_traits<blocked<ndims0>>(GDZ_filter()),
                             ...
                             size_t num_directions, Skeleton&& sk)
  {
    ...
    nested_size_t group_size = pad_tuple<3>(std::make_tuple(num_groups), 1);
    size_t num_locs = fold(level_dims[1], 1, stapl::multiplies<size_t>());

    level_dims[1] = num_locs <= num_groups ? level_dims[1] : group_size;

    sk.span().set_level_dims(level_dims);
  }
};

#endif // KRIPKE_KERNEL_3D_GDZ_H__
