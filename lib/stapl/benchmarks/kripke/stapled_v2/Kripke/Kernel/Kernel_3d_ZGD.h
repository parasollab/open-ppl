...
{
public:
  static constexpr Nesting_Order nesting_val = NEST_ZGD;
...
struct ZGD_filter
{
  ...
  template <typename V>
  typename result<ZGD_filter(V)>::type
  operator()(V&& v) const
  {
    using result_t = typename result<ZGD_filter(V)>::type;
    result_t res;
...
template <>
struct kernel_skeleton<NEST_ZGD>
{
  ...
  static auto make(Corner&& corner, Op&& op)
    STAPL_AUTO_RETURN((wavefront<num_inputs>(
      ...
      corner,
      skeleton_traits<blocked<ndims0>>(ZGD_filter()),
