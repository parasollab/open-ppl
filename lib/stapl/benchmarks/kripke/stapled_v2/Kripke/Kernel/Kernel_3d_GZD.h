...
{
public:
  static constexpr Nesting_Order nesting_val = NEST_GZD;
...
struct GZD_filter
{
  ...
  template <typename V>
  typename result<GZD_filter(V)>::type
  operator()(V&& v) const
  {
    using result_t = typename result<GZD_filter(V)>::type;
    result_t res;
    res.resize(v.size());

    for (size_t g = 0; g < v.size() ; ++g)
    {
      res[g][(size_t)m_direction] = std::move(v[g][(size_t)m_direction]);
    }
...
template <>
struct kernel_skeleton<NEST_GZD>
{
  ...
  STAPL_AUTO_RETURN((
    wavefront<num_inputs>(
      ...
      corner,
      skeleton_traits<blocked<ndims0>>(GZD_filter()),
...
#endif // KRIPKE_KERNEL_3D_GZD_H__
