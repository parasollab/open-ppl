...
#ifndef KRIPKE_BOUNDARY_KERNEL_3D_DGZ_H__
#define KRIPKE_BOUNDARY_KERNEL_3D_DGZ_H__
...
private:
  ...
  using iter_t  = typename plane_t::iterator;

  plane_t* m_plane;
  int      m_offset;
  iter_t   m_iter;
  int      m_idx_max;
  int      m_ndirections_times_ngroups;

public:
  using index_type  = std::tuple<std::size_t, std::size_t, std::size_t>;
  using result_type = double;

  dgz_z_boundary_gen(plane_t* plane,
                     int idx_max,
                     int num_directions,
                     int num_groups,
                     int direction,
                     int group)
    : m_plane(plane),
      m_offset(direction * num_groups + group),
      m_iter(plane->begin() + m_offset),
      m_idx_max(idx_max),
      m_ndirections_times_ngroups(num_directions * num_groups)
  { }
  ...
  operator()(index_type const& idx) const
  {
    const int plane_idx =
     ((std::get<InnerIdx>(idx) * (m_idx_max)) + std::get<OuterIdx>(idx))
       * m_ndirections_times_ngroups;

    return *(m_iter + plane_idx);
  }
  ...
  void define_type(stapl::typer &t)
  {
    t.member(m_plane);
    t.member(m_offset);
    t.transient(m_iter, m_plane->begin() + m_offset);
    t.member(m_idx_max);
    t.member(m_ndirections_times_ngroups);
  }
}; // class dgz_z_boundary_gen
...
public:
  ...
  using value_type =
    typename functor_view_type<
      functor_container<dgz_z_boundary_gen<OuterIdx, InnerIdx>, 3>>::type;

  using result_type = std::array<value_type, 3>;

  dgz_g_boundary_gen(plane_t * plane, int idx_max,
...
public:
  ...
  using result_type =
    typename functor_view_type<
      functor_container<dgz_g_boundary_gen<OuterIdx, InnerIdx>, 1>>::type;

  dgz_d_boundary_gen(plane_t * plane,
                     dimension_t const& dimensions,
...
struct dgz_d_boundary_gen_type
{
  static constexpr direction dir = (direction)axis_num;
  using type = dgz_d_boundary_gen<1, 2>;
};

template<>
struct dgz_d_boundary_gen_type<1>
{
  static constexpr direction dir = (direction)1;
  using type = dgz_d_boundary_gen<0, 2>;
};

template<>
struct dgz_d_boundary_gen_type<2>
{
  static constexpr direction dir = (direction)2;
  using type = dgz_d_boundary_gen<0, 1>;
};
...
template <size_t dir>
struct grid_boundary_traits<dir, NEST_DGZ>
{
  ...
  using boundary_gen_t   = typename dgz_d_boundary_gen_type<dir>::type;
...
#endif // KRIPKE_BOUNDARY_KERNEL_3D_DGZ_H__
