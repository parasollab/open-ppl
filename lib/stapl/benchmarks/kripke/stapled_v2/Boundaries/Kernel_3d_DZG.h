...
#ifndef KRIPKE_BOUNDARY_KERNEL_3D_DZG_H__
#define KRIPKE_BOUNDARY_KERNEL_3D_DZG_H__

#include "common_headers.hpp"


template<int OuterIdx, int InnerIdx>
class dzg_zg_boundary_gen
{
private:
...
public:
  ...
  using result_type = lightweight_vector<double>;

  dzg_zg_boundary_gen(plane_t * plane, int idx_max,
...
}; // class dzg_zd_boundary_gen


template <int OuterIdx, int InnerIdx>
class dzg_d_boundary_gen
{
private:
  using plane_t      = std::vector<double>;
  using dimenstion_t = std::tuple<std::size_t, std::size_t, std::size_t>;

  plane_t *    m_plane;
  int          m_idx_max;
  dimenstion_t m_dimensions;
  int          m_num_directions;
  int          m_num_groups;

public:
  // index is the group
  constexpr static size_t dims_num = 1;
  using traversal_type = stapl::index_sequence<0>;
  using index_type  = std::size_t;
  using inner_gen_type = dzg_zg_boundary_gen<OuterIdx, InnerIdx>;
  using value_type  =
    typename functor_view_type<functor_container<inner_gen_type, 3>>::type;
  ...
  result_type operator()(index_type const& idx) const
  {
    ...
    // result_type res = {gen_view, gen_view, gen_view};
    return {gen_view, gen_view, gen_view};
    // return res;
  }
...
struct dzg_d_boundary_gen_type
{
  static constexpr direction dir = (direction)axis_num;
  using type = dzg_d_boundary_gen<1, 2>;
};

template<>
struct dzg_d_boundary_gen_type<1>
{
  static constexpr direction dir = (direction)1;
  using type = dzg_d_boundary_gen<0, 2>;
};

template<>
struct dzg_d_boundary_gen_type<2>
{
  static constexpr direction dir = (direction)2;
  using type = dzg_d_boundary_gen<0, 1>;
};
...
template <size_t dir>
struct grid_boundary_traits<dir, NEST_DZG>
{
  ...
  using boundary_gen_t   = typename dzg_d_boundary_gen_type<dir>::type;
                                   ...
                                   size_t num_directions)
  {
    ...
    auto&& boundary_spec =
      make_boundary_spec(num_zones, num_directions, locations);
...
#endif // KRIPKE_BOUNDARY_KERNEL_3D_DZG_H__
