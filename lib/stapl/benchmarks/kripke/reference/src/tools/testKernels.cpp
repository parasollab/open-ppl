...
void testKernel(Input_Variables &input_variables, std::string dumpfile){
  int myid;
  ...
  if (!dumpfile.empty()) {
    ...
    sstm << myid;
    ref_data->dump(dumpfile + "." + kr.name() + "." + sstm.str());
  }

  // Cleanup
  if(myid == 0) {
    if (!dumpfile.empty())
      printf("    -- dumpfile = %s.%s.pid\n",
             dumpfile.c_str(), kr.name().c_str());
    printf("    -- OK\n\n");
  }
 ...
 */
void testKernels(Input_Variables &input_variables, std::string dumpfile){
  // Run LTimes
  testKernel<runLTimes>(input_variables, dumpfile);

  // Run LPlusTimes
  testKernel<runLPlusTimes>(input_variables, dumpfile);

  // Run Sweep
  testKernel<runSweep>(input_variables, dumpfile);
}
