...
void User_Data::dump(std::string dumpfile)
{
  FILE *outfp = fopen(dumpfile.c_str(), "wb");
  
  for(size_t i = 0; i < sigma_tot.size(); ++i)
    fprintf(outfp, "sigma_tot[d%]=%f\n", i, sigma_tot[i]);

  for (int i = 0; i < directions.size(); ++i)
  {
    fprintf(outfp, "directions[%d].x=%f\n", i, directions[i].xcos);
    fprintf(outfp, "directions[%d].y=%f\n", i, directions[i].ycos);
    fprintf(outfp, "directions[%d].z=%f\n", i, directions[i].zcos);
  }

  grid_data->dump(outfp);
