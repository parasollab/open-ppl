...
int SweepSolver (User_Data *user_data)
{
  ...
  for (int iter = 0;iter < user_data->niter;++ iter){
     ...
     */
    {
      ...
      for (int group_set = 0; group_set < user_data->num_group_sets;
           ++group_set) {
...
int SweepSolver_GroupSet (int group_set, User_Data *user_data)
{
  ...
  while (directions_left) {
    ...
    for (int ds = 0; ds < num_direction_sets; ds++) {
      if (k_plane_data[ds] == NULL || j_plane_data[ds] == NULL ||
          i_plane_data[ds] == NULL || swept[ds]) {
        continue;
      }
      ...
      /* Use standard Diamond-Difference sweep */
      {
        ...
        user_data->kernel->sweep(grid_data,
                                 &dir_sets[ds],
                                 i_plane_data[ds],
                                 j_plane_data[ds],
                                 k_plane_data[ds]);
