...
typedef stapl::static_array<point_type>                   points_ct_t;
...
void kmeans(int, int, int, string);
...
stapl::exit_code stapl_main(int argc, char* argv[]) {
   ...
   kmeans(k_clusters, point_count, dimensions, argv[4]);
...
void kmeans(int k_clusters, int point_count, int dimensions, string filename) {
  ...
  points_ct_t points_ct(point_count, point_type(dimensions, 0.0));
  points_vw_t points_vw(points_ct);

  centroids_ct_t centroids =
    kmeans_input(k_clusters, dimensions, filename, points_vw);
...
struct map_wf {
  ...
  result_type operator()(Points points, Centroids centroids) const {
    ...
    return result_type(std::move(new_centroids),
                       std::move(centroid_counts),
                       mean_sq_err);
  }
};
...
struct reduce_wf {
  ...
  result_type operator()(Reference1 lhs, Reference2 rhs) const {
    ...
    centroids_ct_t const& rhs_centroids = stapl::get<0>(rhs_val);
    vector<int>& lhs_counts             = stapl::get<1>(lhs_val);
    vector<int> const& rhs_counts       = stapl::get<1>(rhs_val);
