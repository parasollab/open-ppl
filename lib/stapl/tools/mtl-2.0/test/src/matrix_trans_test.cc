...
template <class Matrix, class Trans>
bool trans_test(const Matrix& A, const Trans& T, std::string test_name)
{
  for (typename Matrix::const_iterator i = A.begin();
       i != A.end(); ++i)
    for (typename Matrix::OneD::const_iterator j = (*i).begin();
	 j != (*i).end(); ++j)
      if (*j != T(j.column(), j.row())) {
	std::cerr << "**** FAILED: (trans test) " << test_name.c_str() << " ****" << std::endl;
	return false;
      }
  return true;
}
...
bool trans_test(const Matrix& A, std::string test_name)
{
  if (trans_test(A, mtl::trans(A), test_name) 
      && trans_test(mtl::trans(A), A, test_name)) {
    std::cout << test_name.c_str() << " passed trans test" << std::endl;
    return true;
  } else
    return false;
}
