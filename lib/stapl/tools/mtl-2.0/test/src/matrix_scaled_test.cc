...
template <class Matrix, class Scaled, class T>
bool scaled_test(const Matrix& A, const Scaled& S, T s, std::string test_name)
{
  for (typename Matrix::const_iterator i = A.begin();
       i != A.end(); ++i)
    for (typename Matrix::OneD::const_iterator j = (*i).begin();
	 j != (*i).end(); ++j)
      if (*j * s != S(j.row(), j.column())) {
	std::cerr << "**** FAILED: (scaled test) " << test_name.c_str()
	     << " ****" << std::endl;
#if !defined(_MSVCPP_)
	std::cerr << *j * s << " != " << S(j.row(), j.column()) << std::endl;
#endif
	return false;
      }
  return true;
...
bool scaled_test(const Matrix& A, std::string test_name)
{
  typedef typename mtl::matrix_traits<Matrix>::value_type T;
  if (scaled_test(A, mtl::scaled(A, T(2)), 2.0, test_name) 
      && scaled_test(mtl::scaled(A, T(2)), A, 0.5, test_name)) {
    std::cout << test_name.c_str() << " passed scaled test" << std::endl;
    return true;
  } else
    return false;
}
