  ...
  template<typename _ForwardIterator, typename _Tp>
      ...
      __try
	{
	  ...
	  if(_M_buffer)
          {
            // Shishir - 10/16/2010
            // STL requires the result of dereferencing a ForwardIterator to be a T ref
            // However, in STAPL dereferencing an iterator results in a proxy.
            // This causes a problem with the following function call, as we pass an
            // unnamed temporary proxy object to a function which expects a T ref
            //std::__uninitialized_construct_range(_M_buffer, _M_buffer + _M_len,
            //                                   *__first
            value_type stapl_temp = *__first;
