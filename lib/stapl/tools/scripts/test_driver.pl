...
sub extract_version_info
{
  ...
  my $file_tests_descriptor = json_tests_descriptor($_);
...
for (@test_files) {
  ...
  my @versions_in_file = extract_version_info(\%known_tests, $_);
...
if ($b_run_all_tests)
{
  ...
  for my $base_file (keys %known_files)
  {
    ...
    for my $file (keys %{$known_files{$base_file}})
    {
...
for my $test (@opt_tests)
{
  ...
  if ($b_run_all_versions) {
    @versions = keys %{$known_tests{$test}};
  }
  else {
