...
void test_wf_of_wf_of_wf(ViewTuple&& in_view, Corner&& start,
                         stapl::index_sequence<index...>)
{
  ...
  auto wf_wf_wf = wavefront(wf_wf, corner);
  auto nested_wf_wf_wf = nest(wf_wf_wf);

  std::string title =
    std::to_string(ndims) + "D wavefront(" +
    std::to_string(ndims) + "D wavefront(" +
    std::to_string(ndims) + "D wavefront(+))) non-coarsened";
  ...
  skeletons::execute(
    ...
    nested_wf_wf_wf, std::get<index>(std::forward<ViewTuple>(in_view))...);
...
template<int dim, typename ViewTuple, typename Corner, std::size_t... index>
void test_wf_of_zip_of_wf(ViewTuple&& in_view, Corner&& start,
                                     stapl::index_sequence<index...>)
{
  auto&& traits = skeleton_traits<stapl::use_default, true>();
  auto&& corner = generate_corner<dim>(start);
  ...
  auto wf_sk          = wavefront(sum_op<dim, value_type>(), corner, traits);
  auto zip_wf         = zip<dim + 1>(wf_sk,
                                     skeleton_traits<
                                       spans::blocked<dim>, true>());
  auto wf_zip_wf      = wavefront(zip_wf, corner);
  auto nest_wf_zip_wf = nest(wf_zip_wf);

  std::string title =
    std::to_string(ndims) + "D wavefront(" +
    std::to_string(ndims) + "D zip(" +
    std::to_string(ndims) + "D wavefront(+))) non-coarsened";
                                     ...
                                     stapl::index_sequence<index...>)
{
  ...
  auto zip_sk         = zip<dim + 1>(sum_op<dim, value_type>(),
                                     skeletons::skeleton_traits<
                                       spans::blocked<dim>, true>());
  auto zip_zip        = zip<dim + 1>(zip_sk,
                                     skeletons::skeleton_traits<
                                       spans::blocked<dim>, true>());
  auto wf_zip_wf      = wavefront(zip_zip, corner);
  auto nest_wf_zip_wf = nest(wf_zip_wf);

  std::string title =
    std::to_string(ndims) + "D wavefront(" +
    std::to_string(ndims) + "D zip(" +
    std::to_string(ndims) + "D zip(+))) non-coarsened";
...
template<int dim, typename ViewTuple, typename Corner, std::size_t... index>
void test_wf_of_wf_of_zip(ViewTuple&& in_view, Corner&& start,
                                     stapl::index_sequence<index...>)
{
  auto&& traits = skeleton_traits<stapl::use_default, true>();
  auto&& corner = generate_corner<dim>(start);
  ...
  auto wf_wf_zip = wavefront(wf_zip, corner);
  auto nested_wf_wf_zip = nest(wf_wf_zip);
  ...
  skeletons::execute(
    skeletons::execution_params(),
    nested_wf_wf_zip, std::get<index>(std::forward<ViewTuple>(in_view))...);
...
void test_3_levels(size_t n_level0, size_t n_level1, size_t n_level2)
{
  ...
  in_view_3level[first_idx][first_idx][first_idx] = 1;

  auto f = position::first;

  auto&& inputs_tuple =
    homogeneous_tuple<ndims+1>(in_view_3level);

  test_wf_of_wf_of_wf<ndims>(inputs_tuple, f,
                             make_index_sequence<ndims + 1>());
  test_wf_of_wf_of_zip<ndims>(inputs_tuple, f,
                              make_index_sequence<ndims + 1>());
  test_wf_of_zip_of_wf<ndims>(inputs_tuple, f,
                              make_index_sequence<ndims + 1>());
  test_wf_of_zip_of_zip<ndims>(inputs_tuple, f,
                               make_index_sequence<ndims + 1>());
}
