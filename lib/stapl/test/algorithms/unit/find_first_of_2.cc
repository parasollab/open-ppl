...
stapl::exit_code stapl_main(int argc, char* argv[])
{
  ...
  //
  // std::vector find_first_of string
  //
  vector_type      vec(m);
  ...
  for (int i = 0; i < 32; ++i)
  {
    ...
    const auto ref3 = find_first_of(view, vector_view);

    const double benchmark_elapsed = benchmark_timer.stop();

    if (ref3 != n-m)
      abort("Failure on find_first_of with std::vector");
    ...
    do_once([i, result]()
    {
      std::cout << "find_first_of(std::vector) Iteration " << i
                << " time=" << result << "\n";
    });
  }

  STAPL_TEST_REPORT(true,
    "Testing find_first_of over array, string in std::vector")

  //
  // stapl::array find_first_of string
  //
  array_type find_first_of_str(m);
  view_type  find_first_of_str_view(find_first_of_str);

  copy(counting_view<value_type>(m, n-m), find_first_of_str_view);
  ...
  for (int i = 0; i < 32; ++i)
  {
    ...
    const auto ref3 = find_first_of(view, find_first_of_str_view);

    const double benchmark_elapsed = benchmark_timer.stop();

    if (ref3 != n-m)
      abort("Failure on find_first_of with stapl::array");
    ...
    do_once([i, result]()
    {
      std::cout << "find_first_of(stapl::array) Iteration " << i
                << " time=" << result << "\n";
    });
  }

  STAPL_TEST_REPORT(true,
    "Testing find_first_of over array, string in stapl::array")
  ...
  **** TO DO AFTER PROMOTION FIX FOR PLISTS - UNCOMMENT BELOW
  ...
  do_once(std::cout<<constant("Testing find_first_of over list:"));
  typedef viewl_t::reference refl_t;
  refl_t refer=find_first_of(viewl,vec_view);
