...
stapl::exit_code stapl_main(int argc, char **argv) {
  ...
  stapl::exit_code code1 = experiment(1, model, "adjfind_algo atom", 
                                      atom_times, atom_labels);
  stapl::exit_code code2 = experiment(2, model, "adjfind_algo stl", 
...
bool run_atom_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  int ndx_p = -1;
  auto ref_p = stapl::adjacent_find( v_vw, eq_atom_pred<atom_tp>() );
  ...
  times[0] += time_delta;
  labels[0] = string("algorithm: adjacent_find, user predicate");
  seconds(time_start);

  // METRIC: apply algorithm
  int ndx = -1;
  auto ref = stapl::adjacent_find( v_vw );
  ...
  time_delta = time_end - time_start;
  times[1] += time_delta;
  labels[1] = string("algorithm: adjacent_find");
...
bool run_stl_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  stapl::map_func( init_block_vec_wf(0,7), v_vw, len_vw );

  seconds(time_start);
 
  // METRIC: apply algorithm
  int ndx_p = -1;
  auto ref_p = stapl::adjacent_find( v_vw, eq_stl_pred<stl_tp>() );
  if ( !stapl::is_null_reference(ref_p) ) {
    ndx_p = stapl::index_of(ref_p);
  }
