...
stapl::exit_code stapl_main(int argc, char **argv) {
  ...
  stapl::exit_code code1 = experiment(1, model, "generate_algo atom", 
...
bool run_atom_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  seconds(time_end);
  double step0_time = time_end - time_start;
  times[0] += time_delta;
  labels[0] = string("algorithm: generate, sequence");
  seconds(time_start);
  ...
  times[1] += time_delta;
  labels[1] = string("algorithm: generate, block");
  seconds(time_start);
  ...
  time_delta = time_end - time_start;
  times[2] += time_delta;
  labels[2] = string("algorithm: generate, random");
