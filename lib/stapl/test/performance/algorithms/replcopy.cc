...
stapl::exit_code stapl_main(int argc, char **argv) {
  ...
  stapl::exit_code code1 = experiment(1, model, "replcopy_algo atom", 
                                      atom_times, atom_labels);
  stapl::exit_code code2 = experiment(2, model, "replcopy_algo stl", 
...
bool run_atom_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  atom_tp old_val = 10;
  atom_tp new_val = 77;
  stapl::replace_copy_if(v_vw, w_vw, odd_atom_pred<atom_tp>(), new_val);

  seconds(time_end);
  time_delta = time_end - time_start;
  times[0] += time_delta;
  labels[0] = string("algorithm: replace_copy_if");
...
bool run_stl_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  stapl::replace_copy_if(v_vw, w_vw, odd_stl_pred<stl_tp>(), new_val);
