...
stapl::exit_code stapl_main(int argc, char **argv) {
  ...
  stapl::exit_code code1 = experiment(1, model, "wtinprod_algo atom", 
...
bool run_atom_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  atom_tp prod_v_w = stapl::weighted_inner_product( v_vw, w_vw, u_vw, init,
                                                   stapl::plus<atom_tp>(),
                                                   stapl::max<atom_tp>() );
  ...
  times[0] += time_delta;
  labels[0] = string("algorithm: weighted_inner_product, user functions");
  seconds(time_start);

  // METRIC: apply algorithm
  atom_tp prod_v_u = stapl::weighted_inner_product( v_vw, w_vw, u_vw, init);

  seconds(time_end);
  time_delta = time_end - time_start;
  times[1] += time_delta;
  labels[1] = string("algorithm: weighted_inner_product");
