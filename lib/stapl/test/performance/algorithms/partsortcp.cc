...
stapl::exit_code stapl_main(int argc, char **argv) {
  ...
  stapl::exit_code code1 = experiment(1, model, "partsortcp_algo atom", 
                                      atom_times, atom_labels);
  stapl::exit_code code2 = experiment(2, model, "partsortcp_algo stl", 
...
bool run_atom_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  times[0] += time_delta;
  labels[0] = string("algorithm: partial_sort_copy");
  seconds(time_start);

  // METRIC: apply algorithm
  auto y_vw = stapl::partial_sort_copy(v_vw, v_vw, stapl::less<atom_tp>() );

  seconds(time_end);
  time_delta = time_end - time_start;
  times[1] += time_delta;
  labels[1] = string("algorithm: partial_sort_copy, user predicate");
  seconds(time_start);
...
bool run_stl_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  // METRIC: apply algorithm
  auto y_vw = stapl::partial_sort_copy(v_vw, v_vw, less_stl_pred<stl_tp>() );
