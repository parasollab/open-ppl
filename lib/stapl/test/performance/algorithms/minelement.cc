...
stapl::exit_code stapl_main(int argc, char **argv) {
  ...
  stapl::exit_code code1 = experiment(1, model, "minelem_algo atom", 
                                      atom_times, atom_labels);
  stapl::exit_code code2 = experiment(2, model, "minelem_algo stl", 
...
bool run_atom_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  int ndx = -1;
  auto ref = stapl::min_element(v_vw);
  ...
  times[0] += time_delta;
  labels[0] = string("algorithm: min_element, user predicate");
  seconds(time_start);

  // METRIC: apply algorithm
  int ndx_c = -1;
  auto ref_c = stapl::min_element(w_vw, less_atom_wf());
  if ( !stapl::is_null_reference(ref_c) ) {
    ndx = stapl::index_of(ref_c);
  }

  seconds(time_end);
  time_delta = time_end - time_start;
  times[1] += time_delta;
  labels[1] = string("algorithm: min_element");
...
bool run_stl_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  // METRIC: apply algorithm
  int ndx_c = -1;
  auto ref_c = stapl::min_element(w_vw, less_stl_pred<stl_tp>());
  if ( !stapl::is_null_reference(ref_c) ) {
  ...
  time_delta = time_end - time_start;
  times[0] += time_delta;
  labels[0] = string("algorithm: min_element");
