...
stapl::exit_code stapl_main(int argc, char **argv) {
  ...
  stapl::exit_code code1 = experiment(1, model, "lexcomp_algo atom", 
                                      atom_times, atom_labels);
  stapl::exit_code code2 = experiment(2, model, "lexcomp_algo stl", 
...
typedef stapl::greater<atom_tp> gt_atom_wf;
...
bool run_atom_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  bool u_test = stapl::lexicographical_compare( u_vw, v_vw, gt_atom_wf() );

  seconds(time_end);
  time_delta = time_end - time_start;
  times[0] += time_delta;
  labels[0] = string("algorithm: lexico_compare, user predicate");
  seconds(time_start);

  // METRIC: apply algorithm
  bool w_test = stapl::lexicographical_compare( u_vw, w_vw);

  seconds(time_end);
  time_delta = time_end - time_start;
  times[1] += time_delta;
  labels[1] = string("algorithm: lexico_compare");
...
bool run_stl_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  stapl::map_func( init_seq_vec_wf(0,2), w_vw, len_vw );
  stapl::map_func( init_seq_vec_wf(1,2), u_vw, len_vw );

  seconds(time_start);
 
  // METRIC: apply algorithm
  bool u_test = stapl::lexicographical_compare( u_vw, v_vw, gt_stl_pred<stl_tp>() );
