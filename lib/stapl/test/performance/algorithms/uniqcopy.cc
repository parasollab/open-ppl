...
stapl::exit_code stapl_main(int argc, char **argv) {
  ...
  stapl::exit_code code1 = experiment(1, model, "uniqcopy_algo atom", 
                                      atom_times, atom_labels);
  stapl::exit_code code2 = experiment(2, model, "uniqcopy_algo stl", 
...
bool run_atom_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  stapl::unique_copy(v_vw, u_vw, eq_atom_pred<atom_tp>());

  seconds(time_end);
  time_delta = time_end - time_start;
  times[0] += time_delta;
  labels[0] = string("algorithm: unique_copy, user predicate");
  seconds(time_start);
  ...
  time_delta = time_end - time_start;
  times[1] += time_delta;
  labels[1] = string("algorithm: unique_copy");
...
bool run_stl_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  stapl::map_func( init_block_vec_wf(10,7), u_vw, len_vw );

  seconds(time_start);
 
  // METRIC: apply algorithm
  stapl::unique_copy(v_vw, u_vw, eq_stl_pred<stl_tp>());

  seconds(time_end);
  time_delta = time_end - time_start;
  times[0] += time_delta;
  labels[0] = string("algorithm: unique_copy");
