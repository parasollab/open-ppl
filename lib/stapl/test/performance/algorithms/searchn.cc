...
stapl::exit_code stapl_main(int argc, char **argv) {
  ...
  stapl::exit_code code1 = experiment(1, model, "searchn_algo atom", 
                                      atom_times, atom_labels);
  stapl::exit_code code2 = experiment(2, model, "searchn_algo stl", 
...
bool run_atom_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  vec_atom_tp v_ct(size), w_ct(size/40);
  vec_atom_vw_tp v_vw(v_ct), w_vw(w_ct);
  ...
  int ndx_p = -1;
  atom_tp val = 6;
  auto ref_p = stapl::search_n( v_vw, rep, val, eq_atom_pred<atom_tp>() );
  ...
  times[0] += time_delta;
  labels[0] = string("algorithm: searchn, user predicate");
  seconds(time_start);

  // METRIC: apply algorithm
  int ndx = -1;
  auto ref = stapl::search_n( w_vw, rep, val );
  ...
  time_delta = time_end - time_start;
  times[1] += time_delta;
  labels[1] = string("algorithm: searchn");
...
bool run_stl_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  // construct container and view over it
  ary_stl_tp v_ct(outer), w_ct(outer/40);
  ary_stl_vw_tp v_vw(v_ct), w_vw(w_ct);

  // initialize containers in parallel
  ary_sz_tp vlen(outer);
  ary_sz_vw_tp vlen_vw(vlen);
  stapl::map_func(roll_wf(), vlen_vw, stapl::make_repeat_view(inner));

  ary_sz_tp wlen(outer/40);
  ary_sz_vw_tp wlen_vw(wlen);
  stapl::map_func(roll_wf(), wlen_vw, stapl::make_repeat_view(inner));

  int rep = 4;
  stapl::map_func( init_iota_vec_wf(0), v_vw, vlen_vw );
  stapl::map_func( init_block_vec_wf(0,rep), w_vw, wlen_vw );

  seconds(time_start);
 
  // METRIC: apply algorithm
  int ndx_p = -1;
  auto val = v_vw[v_vw.size()/2];
  auto ref_p = stapl::search_n( v_vw, rep, val, eq_stl_pred<stl_tp>() );
  if ( !stapl::is_null_reference(ref_p) ) {
    ndx_p = stapl::index_of(ref_p);
  }
  ...
  time_delta = time_end - time_start;
  times[0] += time_delta;
  labels[0] = string("algorithm: searchn");
