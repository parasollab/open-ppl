...
stapl::exit_code stapl_main(int argc, char **argv) {
  ...
  stapl::exit_code code1 = experiment(1, model, "copy_algo atom", 
                                      atom_times, atom_labels);
  stapl::exit_code code2 = experiment(2, model, "copy_algo stl", 
...
bool run_atom_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  times[0] += time_delta;
  labels[0] = string("algorithm: copy");
  seconds(time_start);
  ...
  time_delta = time_end - time_start;
  times[1] += time_delta;
  labels[1] = string("algorithm: copy, length");
  seconds(time_start);
...
bool run_stl_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  stapl::map_func( init_iota_vec_wf(0), y_vw, len_vw );
