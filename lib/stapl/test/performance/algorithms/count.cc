...
stapl::exit_code stapl_main(int argc, char **argv) {
  ...
  stapl::exit_code code1 = experiment(1, model, "count_algo atom", 
                                      atom_times, atom_labels);
  stapl::exit_code code2 = experiment(2, model, "count_algo stl", 
...
bool run_atom_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  auto v_cnt = stapl::count_if(v_vw, even_atom_pred<atom_tp>() );

  seconds(time_end);
  time_delta = time_end - time_start;
  times[0] += time_delta;
  labels[0] = string("algorithm: count_if, user predicate");
  seconds(time_start);
  ...
  time_delta = time_end - time_start;
  times[1] += time_delta;
  labels[1] = string("algorithm: count_if");
...
bool run_stl_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  stapl::map_func( init_seq_vec_wf(10,1), v_vw, len_vw );
  stapl::map_func( init_block_vec_wf(10,7), w_vw, len_vw );

  seconds(time_start);
 
  // METRIC: apply algorithm
  auto v_cnt = stapl::count_if(v_vw, even_stl_pred<stl_tp>() );

  seconds(time_end);
  time_delta = time_end - time_start;
  times[0] += time_delta;
  labels[0] = string("algorithm: count_if");
