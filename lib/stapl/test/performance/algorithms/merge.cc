...
stapl::exit_code stapl_main(int argc, char **argv) {
  ...
  stapl::exit_code code1 = experiment(1, model, "merge_algo atom", 
                                      atom_times, atom_labels);
  stapl::exit_code code2 = experiment(2, model, "merge_algo stl", 
...
bool run_atom_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  vec_atom_tp v_ct(size), w_ct(size), u_ct(size*2);
  ...
  stapl::merge(v_vw, w_vw, u_vw);

  seconds(time_end);
  time_delta = time_end - time_start;
  times[0] += time_delta;
  labels[0] = string("algorithm: merge");
...
bool run_stl_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  // construct container and view over it
  ary_stl_tp v_ct(outer), w_ct(outer), u_ct(outer*2);
