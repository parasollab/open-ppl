...
stapl::exit_code stapl_main(int argc, char **argv) {
  ...
  stapl::exit_code code1 = experiment(1, model, "maxelem_algo atom", 
                                      atom_times, atom_labels);
  stapl::exit_code code2 = experiment(2, model, "maxelem_algo stl", 
...
bool run_atom_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  int ndx = -1;
  auto ref = stapl::max_element(v_vw);
  ...
  times[0] += time_delta;
  labels[0] = string("algorithm, max_element");
  seconds(time_start);

  // METRIC: apply algorithm
  int ndx_c = -1;
  auto ref_c = stapl::max_element(w_vw, less_atom_wf());
  ...
  time_delta = time_end - time_start;
  times[1] += time_delta;
  labels[1] = string("algorithm, max_element, user predicate");
...
bool run_stl_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  // METRIC: apply algorithm
  int ndx_c = -1;
  auto ref_c = stapl::max_element(w_vw, less_stl_pred<stl_tp>());
  if ( !stapl::is_null_reference(ref_c) ) {
    ndx_c = stapl::index_of(ref_c);
  }
