...
stapl::exit_code stapl_main(int argc, char **argv) {
  ...
  stapl::exit_code code1 = experiment(1, model, "partsum_algo atom", 
...
bool run_atom_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  typedef stapl::max<atom_tp> oper;
  ...
  times[0] += time_delta;
  labels[0] = string("algorithm: partial_sum, user function");
  seconds(time_start);
  ...
  time_delta = time_end - time_start;
  times[1] += time_delta;
  labels[1] = string("algorithm: partial_sum");
  seconds(time_start);
