...
stapl::exit_code stapl_main(int argc, char **argv) {
  ...
  stapl::exit_code code1 = experiment(1, model, "isperm_algo atom", 
...
bool run_atom_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  bool test_p = stapl::is_permutation(v_vw, w_vw, eq_atom_pred<atom_tp>() );

  seconds(time_end);
  time_delta = time_end - time_start;
  times[0] += time_delta;
  labels[0] = string("algorithm: is_permutation, user predicate");
  seconds(time_start);

  // METRIC: apply algorithm
  bool test = stapl::is_permutation(w_vw, u_vw);

  seconds(time_end);
  time_delta = time_end - time_start;
  times[1] += time_delta;
  labels[1] = string("algorithm: is_permutation");
