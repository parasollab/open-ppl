...
stapl::exit_code stapl_main(int argc, char **argv) {
  ...
  stapl::exit_code code1 = experiment(1, model, "replace_algo atom", 
                                      atom_times, atom_labels);
  stapl::exit_code code2 = experiment(2, model, "replace_algo stl", 
...
bool run_atom_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  atom_tp old_val = 0;
  atom_tp new_val = 333;
  stapl::replace_if(v_vw, odd_atom_pred<atom_tp>(), new_val);

  seconds(time_end);
  time_delta = time_end - time_start;
  times[0] += time_delta;
  labels[0] = string("algorithm: replace_if");
  seconds(time_start);

  // METRIC: apply algorithm
  old_val = 10;
  new_val = 777;
  stapl::replace(w_vw, old_val, new_val);

  seconds(time_end);
  time_delta = time_end - time_start;
  times[1] += time_delta;
  labels[1] = string("algorithm: replace");
...
bool run_stl_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  // construct container and view over it
  ary_stl_tp v_ct(outer), w_ct(outer);
  ...
  // METRIC: apply algorithm
  auto old_val = v_vw[v_vw.size()/2];
  auto new_val = v_vw[v_vw.size()/4];
  stapl::replace_if(v_vw, odd_stl_pred<stl_tp>(), new_val);
  ...
  // METRIC: apply algorithm
  old_val = w_vw[w_vw.size()/5];
  new_val = w_vw[w_vw.size()/10];
  stapl::replace(w_vw, old_val, new_val);
