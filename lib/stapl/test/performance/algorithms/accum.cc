...
stapl::exit_code stapl_main(int argc, char **argv) {
  ...
  stapl::exit_code code1 = experiment(1, model, "accum_algo atom", 
...
bool run_atom_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  atom_tp v_sum = stapl::accumulate(v_vw, 0, add_atom_wf());

  seconds(time_end);
  time_delta = time_end - time_start;
  times[0] += time_delta;
  labels[0] = string("algorithm: accumulate, user function");
  seconds(time_start);

  // METRIC: apply algorithm
  atom_tp w_sum = stapl::accumulate(v_vw, 0);

  seconds(time_end);
  time_delta = time_end - time_start;
  times[1] += time_delta;
  labels[1] = string("algorithm: accumulate, default function");
