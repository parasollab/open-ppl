...
stapl::exit_code stapl_main(int argc, char **argv) {
  ...
  stapl::exit_code code1 = experiment(1, model, "partpt_algo atom", 
                                      atom_times, atom_labels);
  stapl::exit_code code2 = experiment(2, model, "partpt_algo stl", 
...
bool run_atom_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  int ndx = -1;
  atom_tp cut = v_vw[ v_vw.size() / 3 ];
  auto ref = stapl::partition_point(v_vw, cut_atom_pred<atom_tp>(cut));
  ...
  times[0] += time_delta;
  labels[0] = string("algorithm, partition_point");
...
bool run_stl_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  // METRIC: apply algorithm
  int ndx = -1;
  auto cut = v_vw[ v_vw.size() / 3 ];
  auto ref = stapl::partition_point(v_vw, cut_stl_pred<stl_tp>(cut));
  if ( !stapl::is_null_reference(ref) ) {
    ndx = stapl::index_of(ref);
  }
