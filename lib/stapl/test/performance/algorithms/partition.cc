...
stapl::exit_code stapl_main(int argc, char **argv) {
  ...
  stapl::exit_code code1 = experiment(1, model, "partition_algo atom", 
                                      atom_times, atom_labels);
#ifdef STAPL_BUG_FIXED
  stapl::exit_code code2 = experiment(2, model, "partition_algo stl", 
...
bool run_atom_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  atom_tp piv = v_vw[v_vw.size() * .75];
  // METRIC: apply algorithm
  stapl::partition_copy(u_vw, v_vw, w_vw, pivot_atom_pred<atom_tp>(piv) );

  seconds(time_end);
  time_delta = time_end - time_start;
  times[0] += time_delta;
  labels[0] = string("algorithm: partition_copy, user predicate");
  seconds(time_start);

  // METRIC: apply algorithm
  stapl::partition(u_vw, pivot_atom_pred<atom_tp>(piv) );

  seconds(time_end);
  time_delta = time_end - time_start;
  times[1] += time_delta;
  labels[1] = string("algorithm: partition_copy");
...
#ifdef STAPL_BUG_FIXED
bool run_stl_test(size_t outer, size_t inner, double *times ) {
  ...
  stapl::map_func( init_block_vec_wf(0,7), w_vw, len_vw );
  stapl::map_func( init_rand_vec_wf(), u_vw, len_vw );

  seconds(time_start);
 
#if 0
  stl_tp piv = v_vw[v_vw.size() * .75];
  // METRIC: apply algorithm
  stapl::partition_copy(u_vw, v_vw, w_vw, pivot_stl_pred<stl_tp>(piv) );

  // stapl/utility/tuple.hpp:105
  // error: implicity instantiation of undefined template
  // 'std::tuple_element<0, int>'
#endif

  seconds(time_end);
  time_delta = time_end - time_start;
  times[0] += time_delta;
  labels[0] = string("algorithm: partition_copy");
