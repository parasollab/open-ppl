...
stapl::exit_code stapl_main(int argc, char **argv) {
  ...
  stapl::exit_code code1 = experiment(1, model, "stpart_algo atom", 
                                      atom_times, atom_labels);
  stapl::exit_code code2 = experiment(2, model, "stpart_algo stl", 
...
bool run_atom_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  atom_tp piv = v_vw[v_vw.size()/4];
  size_t count = stapl::stable_partition(v_vw, pivot_atom_pred<atom_tp>(piv));

  seconds(time_end);
  time_delta = time_end - time_start;
  times[0] += time_delta;
  labels[0] = string("algoritm: stable_partition");
...
bool run_stl_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  // METRIC: apply algorithm
  stl_tp piv = v_vw[v_vw.size()/4];
  size_t count = stapl::stable_partition(v_vw, pivot_stl_pred<stl_tp>(piv));
