...
stapl::exit_code stapl_main(int argc, char **argv) {
  ...
  stapl::exit_code code1 = experiment(1, model, "eqrange_algo atom", 
                                      atom_times, atom_labels);
  stapl::exit_code code2 = experiment(2, model, "eqrange_algo stl", 
...
bool run_atom_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  atom_tp v_val = v_vw[v_vw.size()/4];
  atom_tp w_val = w_vw[w_vw.size()/4];

  // METRIC: apply algorithm
  vec_atom_vw_tp x_vw = stapl::equal_range( v_vw, v_val, 
                                            less_atom_pred<atom_tp>() );

  seconds(time_end);
  time_delta = time_end - time_start;
  times[0] += time_delta;
  labels[0] = string("algorithm: equal_range, user predicate");
  seconds(time_start);

  // METRIC: apply algorithm
  vec_atom_vw_tp y_vw = stapl::equal_range( u_vw, w_val );

  seconds(time_end);
  time_delta = time_end - time_start;
  times[1] += time_delta;
  labels[1] = string("algorithm: equal_range");
...
bool run_stl_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  stapl::map_func( init_block_vec_wf(0,7), u_vw, len_vw );

  seconds(time_start);
 
  stl_tp v_val = v_vw[v_vw.size()/4];

  // METRIC: apply algorithm
  auto x_vw = stapl::equal_range( v_vw, v_val, 
                                            less_stl_pred<stl_tp>() );
