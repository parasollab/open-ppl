...
stapl::exit_code stapl_main(int argc, char **argv) {
  ...
  stapl::exit_code code1 = experiment(1, model, "findfirst_algo atom", 
                                      atom_times, atom_labels);
#ifdef STAPL_BUG_FIXED
  stapl::exit_code code2 = experiment(2, model, "findfirst_algo stl", 
...
bool run_atom_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  int ndx = -1;
  auto ref = stapl::find_first_of(v_vw, w_vw);
  ...
  times[0] += time_delta;
  labels[0] = string("algorithm: find_first_of");
  seconds(time_start);
 
#if 0
  // METRIC: apply algorithm
  ndx = -1;
  ref = stapl::find_first_of(v_vw, w_vw, stapl::equal_to<atom_tp>());
  if ( !stapl::is_null_reference(ref) ) {
    ndx = stapl::index_of(ref);
  }
#endif
  ...
  time_delta = time_end - time_start;
  times[1] += time_delta;
  labels[1] = string("algorithm: find_first_of, user predicate");
...
bool run_stl_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  // METRIC: apply algorithm
  int ndx = -1;
  auto ref = stapl::find_first_of(v_vw, w_vw, eq_stl_pred<stl_tp>());
  if ( !stapl::is_null_reference(ref) ) {
    ndx = stapl::index_of(ref);
  }
  ...
  time_delta = time_end - time_start;
  times[0] += time_delta;
  labels[0] = string("algorithm: find_first_of, user predicate");
