...
stapl::exit_code stapl_main(int argc, char **argv) {
  ...
  stapl::exit_code code1 = experiment(1, model, "ispart_algo atom", 
                                      atom_times, atom_labels);
  stapl::exit_code code2 = experiment(2, model, "ispart_algo stl", 
...
bool run_atom_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  atom_tp mid = v_vw[v_vw.size()/2];
  bool test = stapl::is_partitioned(v_vw, half_atom_pred<atom_tp>(mid));

  seconds(time_end);
  time_delta = time_end - time_start;
  times[0] += time_delta;
  labels[0] = string("algorithm: is_partitioned");
...
bool run_stl_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  // METRIC: apply algorithm
  auto mid = v_vw[v_vw.size()/2];
  bool test = stapl::is_partitioned(v_vw, half_stl_pred<stl_tp>(mid));
