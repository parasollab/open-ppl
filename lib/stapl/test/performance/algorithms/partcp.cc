...
stapl::exit_code stapl_main(int argc, char **argv) {
  ...
  stapl::exit_code code1 = experiment(1, model, "partcp_algo atom", 
                                      atom_times, atom_labels);
#ifdef STAPL_BUG_FIXED
  stapl::exit_code code2 = experiment(2, model, "partcp_algo stl", 
...
bool run_atom_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  stapl::partition_copy(v_vw, u_vw, w_vw, even_atom_pred<atom_tp>() );
...
#if 0
  // METRIC: apply algorithm
  stapl::partition_copy(v_vw, u_vw, w_vw, even_stl_pred<stl_tp>() );
