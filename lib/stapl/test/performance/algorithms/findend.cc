...
stapl::exit_code stapl_main(int argc, char **argv) {
  ...
  stapl::exit_code code1 = experiment(1, model, "findend_algo atom", 
                                      atom_times, atom_labels);
#ifdef STAPL_BUG_FIXED
  stapl::exit_code code2 = experiment(2, model, "findend_algo stl", 
...
bool run_atom_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  int ndx = -1;
  auto ref = stapl::find_end(v_vw, w_vw);
  ...
  times[0] += time_delta;
  labels[0] = string("algorithm: find_end");
  seconds(time_start);

#if 0
  // METRIC: apply algorithm
  ndx = -1;
  ref = stapl::find_end(v_vw, w_vw, eq_atom_pred<atom_tp>());
  if ( !stapl::is_null_reference(ref) ) {
    ndx = stapl::index_of(ref);
  }
#endif
  ...
  time_delta = time_end - time_start;
  times[1] += time_delta;
  labels[1] = string("algorithm: find_end, user predicate");
...
bool run_stl_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  // METRIC: apply algorithm
  int ndx = -1;
  auto ref = stapl::find_end(v_vw, w_vw, eq_stl_pred<stl_tp>());
  if ( !stapl::is_null_reference(ref) ) {
    ndx = stapl::index_of(ref);
  }
  ...
  time_delta = time_end - time_start;
  times[0] += time_delta;
  labels[0] = string("algorithm: find_end, user predicate");
