...
stapl::exit_code stapl_main(int argc, char **argv) {
  ...
  stapl::exit_code code1 = experiment(1, model, "transform_algo atom", 
                                      atom_times, atom_labels);
  stapl::exit_code code2 = experiment(2, model, "transform_algo stl", 
...
typedef stapl::negate<atom_tp> neg_atom_wf;
typedef stapl::min<atom_tp> min_atom_wf;
...
bool run_atom_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  vec_atom_tp v_ct(size), w_ct(size);
  vec_atom_vw_tp v_vw(v_ct), w_vw(w_ct);

  vec_atom_tp x_ct(size), y_ct(size), z_ct(size);
  vec_atom_vw_tp x_vw(x_ct), y_vw(y_ct), z_vw(z_ct);
  ...
  stapl::transform(v_vw, w_vw, neg_atom_wf());

  seconds(time_end);
  time_delta = time_end - time_start;
  times[0] += time_delta;
  labels[0] = string("algorithm: transform, unary");
  seconds(time_start);

  // METRIC: apply algorithm
  stapl::transform(x_vw, y_vw, z_vw, max_atom_wf());

  seconds(time_end);
  time_delta = time_end - time_start;
  times[1] += time_delta;
  labels[1] = string("algorithm: transform, binary");
...
bool run_stl_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  // construct container and view over it
  ary_stl_tp v_ct(outer), w_ct(outer), x_ct(outer), y_ct(outer), z_ct(outer);
  ary_stl_vw_tp v_vw(v_ct), w_vw(w_ct), x_vw(x_ct), y_vw(y_ct), z_vw(z_ct);
  ...
  stapl::map_func( init_seq_vec_wf(0,10), x_vw, len_vw );
  stapl::map_func( init_block_vec_wf(0,7), y_vw, len_vw );

  seconds(time_start);
 
  // METRIC: apply algorithm
  stapl::transform(v_vw, w_vw, neg_stl_wf<stl_tp>());
  ...
  // METRIC: apply algorithm
  stapl::transform(x_vw, y_vw, z_vw, max_stl_wf<stl_tp>());
