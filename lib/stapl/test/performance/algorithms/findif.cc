...
stapl::exit_code stapl_main(int argc, char **argv) {
  ...
  stapl::exit_code code1 = experiment(1, model, "findif_algo atom", 
                                      atom_times, atom_labels);
  stapl::exit_code code2 = experiment(2, model, "findif_algo stl", 
...
typedef stapl::not_equal_to<atom_tp> neq_atom_wf;
...
bool run_atom_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  int ndx = -1;
  auto ref = stapl::find_if(v_vw, bind(eq_atom_wf(),_1,100) );
  ...
  times[0] += time_delta;
  labels[0] = string("algorithm: find_if");
  seconds(time_start);

  // METRIC: apply algorithm
  int ndx_n = -1;
  auto ref_n = stapl::find_if_not(w_vw, bind(neq_atom_wf(),_1,100) );
  ...
  time_delta = time_end - time_start;
  times[1] += time_delta;
  labels[1] = string("algorithm: find_if_not");
  seconds(time_start);
...
bool run_stl_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  // METRIC: apply algorithm
  auto w_val = w_vw[w_vw.size()/2];
  int ndx_n = -1;
  auto ref_n = stapl::find_if_not(w_vw,
                                  bind(neq_stl_pred<stl_tp>(),_1,w_val) );
  if ( !stapl::is_null_reference(ref_n) ) {
    ndx_n = stapl:: index_of(ref_n);
  }
  ...
  time_delta = time_end - time_start;
  times[0] += time_delta;
  labels[0] = string("algorithm: find_if_not");
