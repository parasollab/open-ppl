...
stapl::exit_code stapl_main(int argc, char **argv) {
  ...
  stapl::exit_code code1 = experiment(1, model, "allanynone_algo atom", 
                                      atom_times, atom_labels);
  stapl::exit_code code2 = experiment(2, model, "allanynone_algo stl", 
...
bool run_atom_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  int base = 0;
  int step = 10;
  typedef stapl::sequence<atom_tp> step_wf;
  ...
  all_test = stapl::all_of(v_vw, even_atom_pred<atom_tp>() );

  seconds(time_end);
  time_delta = time_end - time_start;
  times[0] += time_delta;
  labels[0] = string("algorithm: all_of");
  seconds(time_start);

  // METRIC: apply algorithm
  none_test = stapl::none_of(w_vw, neg_atom_pred<atom_tp>() );

  seconds(time_end);
  time_delta = time_end - time_start;
  times[1] += time_delta;
  labels[1] = string("algorithm: none_of");
  seconds(time_start);

  // METRIC: apply algorithm
  any_test = stapl::any_of(u_vw, odd_atom_pred<atom_tp>() );

  seconds(time_end);
  time_delta = time_end - time_start;
  times[2] += time_delta;
  labels[2] = string("algorithm: any_of");
...
bool run_stl_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  stapl::map_func( init_iota_vec_wf(0), u_vw, len_vw );
  ...
  // METRIC: apply algorithm
  bool all_test = stapl::all_of(v_vw, even_stl_pred<stl_tp>() );
  ...
  // METRIC: apply algorithm
  bool none_test = stapl::none_of(w_vw, neg_stl_pred<stl_tp>() );
  ...
  // METRIC: apply algorithm
  bool any_test = stapl::any_of(u_vw, odd_stl_pred<stl_tp>() );
