...
stapl::exit_code stapl_main(int argc, char **argv) {
  ...
  stapl::exit_code code1 = experiment(1, model, "find_algo atom", 
                                      atom_times, atom_labels);
  stapl::exit_code code2 = experiment(2, model, "find_algo stl", 
...
bool run_atom_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  const atom_tp val = v_vw[v_vw.size()/2];
  int ndx = -1;

  auto ref = stapl::find(v_vw, val);
  ...
  times[0] += time_delta;
  labels[0] = string("algorithm: find");
...
bool run_stl_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  // METRIC: apply algorithm
  const stl_tp val = v_vw[v_vw.size()/2];
  int ndx = -1;
