...
stapl::exit_code stapl_main(int argc, char **argv) {
  ...
  stapl::exit_code code1 = experiment(1, model, "equal_algo atom", 
                                      atom_times, atom_labels);
  stapl::exit_code code2 = experiment(2, model, "equal_algo stl", 
...
bool run_atom_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  bool test_p = stapl::equal(v_vw, w_vw, eq_atom_pred<atom_tp>() );

  seconds(time_end);
  time_delta = time_end - time_start;
  times[0] += time_delta;
  labels[0] = string("algorithm: equal");
...
bool run_stl_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  stapl::map_func( init_seq_vec_wf(0,1), w_vw, len_vw );
  stapl::map_func( init_seq_vec_wf(0,2), u_vw, len_vw );

  seconds(time_start);
 
  // METRIC: apply algorithm
  bool test_p = stapl::equal(v_vw, w_vw, eq_stl_pred<stl_tp>() );
