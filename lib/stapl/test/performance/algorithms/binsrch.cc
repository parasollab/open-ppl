...
stapl::exit_code stapl_main(int argc, char **argv) {
  ...
  stapl::exit_code code1 = experiment(1, model, "binsrch_algo atom", 
                                      atom_times, atom_labels);
  stapl::exit_code code2 = experiment(2, model, "binsrch_algo stl", 
...
bool run_atom_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  bool v_test = stapl::binary_search(v_vw, val, less_atom_pred<atom_tp>() );

  seconds(time_end);
  time_delta = time_end - time_start;
  times[0] += time_delta;
  labels[0] = string("algorithm: binary_search, user predicate");
  seconds(time_start);
  ...
  time_delta = time_end - time_start;
  times[1] += time_delta;
  labels[1] = string("algorithm: binary_search");
...
bool run_stl_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  stapl::map_func( init_seq_vec_wf(1,2), w_vw, len_vw );

  seconds(time_start);
 
  // METRIC: apply algorithm
  auto val = v_vw[ v_vw.size()*.75 ];
  bool v_test = stapl::binary_search(v_vw, val, less_stl_pred<stl_tp>() );
