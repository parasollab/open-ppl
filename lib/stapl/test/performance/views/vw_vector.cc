...
stapl::exit_code stapl_main(int argc, char **argv) {
  ...
  stapl::exit_code code1 = experiment(1, model, "vector_vw atom", 
                           atom_times, atom_labels);
  stapl::exit_code code2 = experiment(2, model, "vector_vw stl", 
                           stl_times, stl_labels);
#ifdef NESTED_ACTIVE
  stapl::exit_code code3 = experiment(3, model, "vector_vw nest", 
...
bool run_atom_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  times[0] += time_delta;
  labels[0] = string("view: vector, construct view");
  ...
  time_delta = time_end - time_start;
  times[1] += time_delta;
  labels[1] = string("view: vector, initialize elements");

  seconds(time_start);
  // METRIC: process elements in parallel
  atom_tp max_val;
  max_val = stapl::map_reduce( stapl::identity<atom_tp>(),
                               stapl::max<atom_tp>(), v_vw );

  seconds(time_end);
  time_delta = time_end - time_start;
  times[2] += time_delta;
  labels[2] = string("view: vector, process elements");

  seconds(time_start);
  // METRIC: append elements
  stapl::do_once([&]() {
    v_vw.push_back(1001);
    v_vw.push_back(2002);
    v_vw.push_back(3003);
  ...
  time_delta = time_end - time_start;
  times[3] += time_delta;
  labels[3] = string("view: vector, append elements");
  ...
  time_delta = time_end - time_start;
  times[4] += time_delta;
  labels[4] = string("view: vector, process more elements");
...
bool run_stl_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  stapl::map_func( init_stl_vec_wf(), v_vw, len_vw );

  seconds(time_end);
  time_delta = time_end - time_start;
  times[1] += time_delta;
  labels[1] = string("view: vector, initialize containers");
  seconds(time_start);

  // METRIC: process elements in parallel
  atom_tp sum = stapl::map_reduce( map_sz_wf(), add_sz_wf(), v_vw );
...
bool run_nest_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  ary_sz_vw_tp len_vw(len);
  stapl::map_func(roll_wf(), len_vw, stapl::make_repeat_view(inner));
  vec_nest_tp v_ct(len_vw);

  seconds(time_start);

  // metric: construct view over container
  vec_nest_vw_tp v_vw(v_ct);
