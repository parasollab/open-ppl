...
stapl::exit_code stapl_main(int argc, char **argv) {
  ...
  stapl::exit_code code1 = experiment(1, model, "reverse_vw atom", 
                           atom_times, atom_labels);
  stapl::exit_code code2 = experiment(2, model, "reverse_vw stl", 
                           stl_times, stl_labels);
#ifdef NESTED_ACTIVE
  stapl::exit_code code3 = experiment(3, model, "reverse_vw nest", 
...
typedef stapl::view_impl::reverse_view<ary_atom_vw_tp> rev_ary_atom_vw_tp;
typedef stapl::view_impl::reverse_view<rev_ary_atom_vw_tp> rev_rev_ary_atom_vw_tp;
...
bool run_atom_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  // construct container
  ary_atom_tp a_ct(size), b_ct(size), c_ct(size);

  // construct view over container
  ary_atom_vw_tp a_ary_vw(a_ct), b_ary_vw(b_ct), c_ary_vw(c_ct);

  seconds(time_start);
  // METRIC: construct view
  rev_ary_atom_vw_tp a_rev_vw = rev_ary_atom_vw_tp(a_ary_vw);

  seconds(time_end);
  time_delta = time_end - time_start;
  times[0] += time_delta;
  labels[0] = string("view: reverse, construct view");
  ...
  stapl::scan(a_ary_vw, b_ary_vw, add_atom_wf(), false);
  stapl::scan(a_rev_vw, c_ary_vw, add_atom_wf(), false);

  seconds(time_end);
  time_delta = time_end - time_start;
  times[1] += time_delta;
  labels[1] = string("view: reverse, process elements");
...
typedef stapl::view_impl::reverse_view<ary_stl_vw_tp> rev_ary_stl_vw_tp;
typedef stapl::view_impl::reverse_view<rev_ary_stl_vw_tp> rev_rev_ary_stl_vw_tp;
...
bool run_stl_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  // construct container
  ary_stl_tp a_ct(outer), b_ct(outer), c_ct(outer);
  // construct view over container
  ary_stl_vw_tp a_vw(a_ct), b_vw(b_ct), c_vw(c_ct);

  seconds(time_start);
  // METRIC: construct view 
  rev_ary_stl_vw_tp a_rev_vw = rev_ary_stl_vw_tp(a_vw);
...
typedef stapl::view_impl::reverse_view<ary_nest_vw_tp> rev_ary_nest_vw_tp;
typedef stapl::view_impl::reverse_view<rev_ary_nest_vw_tp> rev_rev_ary_nest_vw_tp;
...
bool run_nest_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  time_delta = time_end - time_start;
  times[2] += time_delta;
  labels[2] = string("view: reverse, process elements");
