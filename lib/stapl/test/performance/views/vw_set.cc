...
stapl::exit_code stapl_main(int argc, char **argv) {
  ...
  stapl::exit_code code1 = experiment(1, model, "set_vw atom", 
                           atom_times, atom_labels);
  stapl::exit_code code2 = experiment(2, model, "set_vw stl", 
                           stl_times, stl_labels);
#ifdef NESTED_ACTIVE
  stapl::exit_code code3 = experiment(3, model, "set_vw nest", 
...
typedef stapl::unordered_set<atom_tp> unord_set_atom_tp;
typedef stapl::set_view<set_atom_tp> set_atom_vw_tp;
typedef stapl::set_view<unord_set_atom_tp> unord_set_atom_vw_tp;
...
bool run_atom_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  // construct container
  ary_atom_tp key_ct(init_count+20);
  ary_atom_vw_tp key_vw(key_ct);
  ...
  stapl::do_once([&]() {
    ...
    for ( ; i<init_count+20; i++ ) {
      size_t j = (i < init_count) ? i : i % init_count;
      key_vw[i] = prime_nums[j];
    }
  });
  stapl::rmi_fence();
  ...
  set_atom_vw_tp t_vw(s_ct);

  time_delta = time_end - time_start;
  times[0] += time_delta;
  labels[0] = string("view: set, construct view");

  seconds(time_start);
  // METRIC: initialize container in parallel
  stapl::map_func( init_set_wf(), 
                   key_vw, stapl::make_repeat_view(t_vw) );
  set_atom_vw_tp s_vw(s_ct);

  time_delta = time_end - time_start;
  times[1] += time_delta;
  labels[1] = string("view: set, initialize elements");

  seconds(time_start);
  // METRIC: process elements in parallel
  atom_tp sum = stapl::map_reduce( stapl::identity<atom_tp>(),
                                  stapl::plus<atom_tp>(), s_vw );

  seconds(time_end);
  time_delta = time_end - time_start;
  times[2] += time_delta;
  labels[2] = string("view: set, process elements");

  return true;
}

///////////////////////////////////////////////////////////////////////////
// plot: set of STL vectors
///////////////////////////////////////////////////////////////////////////
...
bool run_stl_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  time_delta = time_end - time_start;
  times[1] += time_delta;
  labels[1] = string("view: set, initialize containers");
