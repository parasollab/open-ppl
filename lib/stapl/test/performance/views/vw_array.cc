...
stapl::exit_code stapl_main(int argc, char **argv) {
  ...
  stapl::exit_code code1 = experiment(1, model, "array_vw atom", 
                           atom_times, atom_labels);
  stapl::exit_code code2 = experiment(2, model, "array_vw stl", 
                           stl_times, stl_labels);
#ifdef NESTED_ACTIVE
  stapl::exit_code code3 = experiment(3, model, "array_vw nest", 
...
bool run_atom_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  times[0] += time_delta;
  labels[0] = string("view: array, construct view");
  seconds(time_start);
  ...
  time_delta = time_end - time_start;
  times[1] += time_delta;
  labels[1] = string("view: array, initialize container");
  seconds(time_start);
  ...
  time_delta = time_end - time_start;
  times[2] += time_delta;
  labels[2] = string("view: array, process elements");
...
bool run_stl_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  time_delta = time_end - time_start;
  times[2] += time_delta;
  labels[2] = string("container: array, process elements");
  seconds(time_start);
...
bool run_nest_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  time_delta = time_end - time_start;
  times[1] += time_delta;
  labels[1] = string("view: array, initialize containers");
  seconds(time_start);
