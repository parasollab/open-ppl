...
stapl::exit_code stapl_main(int argc, char **argv) {
  ...
  stapl::exit_code code1 = experiment(1, model, "native_vw atom", 
                           atom_times, atom_labels);
  stapl::exit_code code2 = experiment(2, model, "native_vw stl", 
                           stl_times, stl_labels);
#ifdef NESTED_ACTIVE
  stapl::exit_code code3 = experiment(3, model, "native_vw nest", 
...
///////////////////////////////////////////////////////////////////////////
// PLOT: native view over container of atoms
///////////////////////////////////////////////////////////////////////////
...
bool run_atom_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  // construct container
  vec_atom_tp a_ct(size);
  size_t loc_nums = a_ct.get_num_locations();
  vec_atom_tp b_ct(loc_nums);

  seconds(time_start);

  // METRIC: construct view over container
  vec_atom_vw_tp a_vw(a_ct), b_vw(b_ct);

  seconds(time_end);
  time_delta = time_end - time_start;
  times[0] += time_delta;
  labels[0] = string("view: native, construct view");
  seconds(time_start);
  ...
  time_delta = time_end - time_start;
  times[1] += time_delta;
  labels[1] = string("view: native, initialize elements");
  seconds(time_start);
  ...
  time_delta = time_end - time_start;
  times[2] += time_delta;
  labels[2] = string("view: native, initialize containers");

  return true;
}

///////////////////////////////////////////////////////////////////////////
// PLOT: native view over container of STL vectors
///////////////////////////////////////////////////////////////////////////
...
bool run_stl_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  time_delta = time_end - time_start;
  times[1] += time_delta;
  labels[1] = string("view: native, initialize containers");
  seconds(time_start);
  ...
  time_delta = time_end - time_start;
  times[2] += time_delta;
  labels[2] = string("view: native, process elements");
