...
stapl::exit_code stapl_main(int argc, char **argv) {
  ...
  stapl::exit_code code1 = experiment(1, model, "arrayro_vw atom", 
                           atom_times, atom_labels);
  stapl::exit_code code2 = experiment(2, model, "arrayro_vw stl", 
                           stl_times, stl_labels);
#ifdef NESTED_ACTIVE
  stapl::exit_code code3 = experiment(3, model, "arrayro_vw nest", 
...
///////////////////////////////////////////////////////////////////////////
// PLOT: array read-only view over container of atoms
///////////////////////////////////////////////////////////////////////////

typedef int atom_tp;
typedef stapl::array<atom_tp> ary_atom_tp;
typedef stapl::array_ro_view<ary_atom_tp> ary_atom_vw_tp;
...
bool run_atom_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  times[0] += time_delta;
  labels[0] = string("view: arrayro, construct view");
  seconds(time_start);
  ...
  time_delta = time_end - time_start;
  times[1] += time_delta;
  labels[1] = string("view: arrayro, initialize container");
  seconds(time_start);

  // METRIC: process elements in parallel
  atom_tp sum = stapl::map_reduce( stapl::identity<atom_tp>(),
                                  stapl::plus<atom_tp>(), a_vw );

  seconds(time_end);
  time_delta = time_end - time_start;
  times[2] += time_delta;
  labels[2] = string("view: arrayro, process elements");

  return true;
}

///////////////////////////////////////////////////////////////////////////
// PLOT: array read-only view over container of STL vectors
///////////////////////////////////////////////////////////////////////////

typedef vector<atom_tp> stl_tp;
typedef stapl::array<stl_tp> ary_stl_tp;
typedef stapl::array_ro_view<ary_stl_tp> ary_stl_vw_tp;
...
bool run_stl_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  time_delta = time_end - time_start;
  times[1] += time_delta;
  labels[1] = string("view: arrayro, initialize containers");
  seconds(time_start);
...
///////////////////////////////////////////////////////////////////////////
// plot: array read-only view over container of STAPL vectors
///////////////////////////////////////////////////////////////////////////

#ifdef NESTED_ACTIVE
typedef stapl::vector<atom_tp> nest_tp;
typedef stapl::array<nest_tp> ary_nest_tp;
typedef stapl::array_ro_view<ary_nest_tp> ary_nest_vw_tp;
