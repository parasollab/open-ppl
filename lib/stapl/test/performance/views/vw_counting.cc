...
stapl::exit_code stapl_main(int argc, char **argv) {
  ...
  stapl::exit_code code1 = experiment(1, model, "counting_vw atom", 
                           atom_times, atom_labels);
  stapl::exit_code code2 = experiment(2, model, "counting_vw stl", 
                           stl_times, stl_labels);
#ifdef NESTED_ACTIVE
  stapl::exit_code code3 = experiment(3, model, "counting_vw nest", 
...
bool run_atom_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  // construct container
  vec_atom_tp a_ct(size), b_ct(size);
  ...
  times[0] += time_delta;
  labels[0] = string("view: counting, construct view");
  ...
  stapl::map_func(add_wf(), a_vw, stapl::counting_view<atom_tp>(a_vw.size()),
                            b_vw );

  seconds(time_end);
  time_delta = time_end - time_start;
  times[1] += time_delta;
  labels[1] = string("view: counting, process elements");
