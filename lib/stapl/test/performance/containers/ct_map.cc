...
stapl::exit_code stapl_main(int argc, char **argv) {
  ...
  stapl::exit_code code1 = experiment(1, model, "map_ct atom", 
                           atom_times, atom_labels);
  stapl::exit_code code2 = experiment(2, model, "map_ct stl", 
                           stl_times, stl_labels);
#ifdef NESTED_ACTIVE
  stapl::exit_code code3 = experiment(3, model, "map_ct nest", 
...
typedef stapl::map<atom_tp, atom_tp, stapl::less<atom_tp>,
...
bool run_atom_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  times[0] += time_delta;
  labels[0] = string("container: map, construct, defaults");
  seconds(time_start);
  ...
  time_delta = time_end - time_start;
  times[1] += time_delta;
  labels[1] = string("container: map, construct, non-default distribution");
  seconds(time_start);
  ...
  time_delta = time_end - time_start;
  times[2] += time_delta;
  labels[2] = string("container: map, initialize");
  seconds(time_start);
  ...
  time_delta = time_end - time_start;
  times[3] += time_delta;
  labels[3] = string("container: map, find element");
  seconds(time_start);

  // METRIC: redistribute container
  dist_spec_tp new_dist = stapl::block(outer,10);
#ifdef STAPL_OR_TEST_BUG
  q_ct.redistribute(new_dist);
#endif
 
  seconds(time_end);
  time_delta = time_end - time_start;
  times[4] += time_delta;
  labels[4] = string("container: map, redistribute");
...
typedef stapl::map<atom_tp, stl_tp, stapl::less<atom_tp>,
        stapl::view_based_partition<dist_spec_tp>,
...
typedef stapl::map<atom_tp, nest_tp, stapl::less<atom_tp>,
        stapl::view_based_partition<dist_spec_tp>,
...
bool run_nest_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  time_delta = time_end - time_start;
  times[3] += time_delta;
  labels[3] = string("container: map, redistribute");
