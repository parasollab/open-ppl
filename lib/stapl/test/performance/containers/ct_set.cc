...
stapl::exit_code stapl_main(int argc, char **argv) {
  ...
  stapl::exit_code code1 = experiment(1, model, "set_ct atom", 
                           atom_times, atom_labels);
#ifdef SUPPORTED_BY_STAPL
  stapl::exit_code code2 = experiment(2, model, "set_ct stl", 
                           stl_times, stl_labels);
#ifdef NESTED_ACTIVE
  stapl::exit_code code3 = experiment(3, model, "set_ct nest", 
...
bool run_atom_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  times[0] += time_delta;
  labels[0] = string("container: set, construct, defaults");
  seconds(time_start);
  ...
  time_delta = time_end - time_start;
  times[1] += time_delta;
  labels[1] = string("container: set, construct, non-default distribution");
  seconds(time_start);
  ...
  for ( int i=0; i<outer; i++ ) {
    ...
    s_ct.insert(elem);
#ifdef STAPL_OR_TEST_BUG
    t_ct.insert(i+prime_nums[i%init_count]);
#endif
  }
  stapl::rmi_fence();
  ...
  time_delta = time_end - time_start;
  times[2] += time_delta;
  labels[2] = string("container: set, initialize");
  seconds(time_start);
  ...
  time_delta = time_end - time_start;
  times[3] += time_delta;
  labels[3] = string("container: set, find element");
