...
stapl::exit_code stapl_main(int argc, char **argv) {
  ...
  stapl::exit_code code1 = experiment(1, model, "list_ct atom", 
                           atom_times, atom_labels);
  stapl::exit_code code2 = experiment(2, model, "list_ct stl", 
                           stl_times, stl_labels);
#ifdef NESTED_ACTIVE
  stapl::exit_code code3 = experiment(3, model, "list_ct nest", 
                            ...
                            double *times, string *labels)
{
  ...
  size_t outer_size = total_size / 1000 ;
...
bool run_atom_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  list_atom_tp l_ct(outer);

  seconds(time_end);
  time_delta = time_end - time_start;
  times[0] += time_delta;
  labels[0] = string("container: list, construct, defaults");
  seconds(time_start);

  // METRIC: construct container with non-default initializer
  list_atom_tp m_ct(outer, 314159);

  seconds(time_end);
  time_delta = time_end - time_start;
  times[1] += time_delta;
  labels[1] = string("container: list, construct, non-default initializer");
  seconds(time_start);

  stapl::do_once([&]() {
    // append elements with sequential loop
    for ( int i=0; i<10; i++ ) {
      l_ct.push_back(prime_nums[i%init_count]);
    }

    // append elements with sequential loop
    for ( int i=0; i<20; i++ ) {
      l_ct.push_back(fibo[i%20]);
    }
  });
  ...
  time_delta = time_end - time_start;
  times[2] += time_delta;
  labels[2] = string("container: list, append elements");
  seconds(time_start);
  ...
  time_delta = time_end - time_start;
  times[3] += time_delta;
  labels[3] = string("container: list, construct, non-default distribution");
...
bool run_stl_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  // METRIC: construct container with non-default initializer
  list_stl_tp m_ct(outer, vector<atom_tp>(271828) );
  ...
  stapl::do_once([&]() {
    ...
    for ( int i=0; i<20; i++ ) {
#ifdef STAPL_OR_TEST_BUG
      l_ct.push_back( vector<atom_tp>(inner,prime_nums[i%init_count]) );
#endif
    }
    ...
    for ( int i=0; i<20; i++ ) {
#ifdef STAPL_OR_TEST_BUG
      l_ct.push_back( vector<atom_tp>(inner,fibo[i%20]) );
#endif
    }
  });
...
bool run_nest_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  time_delta = time_end - time_start;
  times[0] += time_delta;
  labels[0] = string("container: list, construct");
  seconds(time_start);
