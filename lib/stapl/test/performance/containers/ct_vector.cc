...
stapl::exit_code stapl_main(int argc, char **argv) {
  ...
  stapl::exit_code code1 = experiment(1, model, "vector_ct atom", 
                           atom_times, atom_labels);
  stapl::exit_code code2 = experiment(2, model, "vector_ct stl", 
                           stl_times, stl_labels);
#ifdef NESTED_ACTIVE
  stapl::exit_code code3 = experiment(3, model, "vector_ct nest", 
...
///////////////////////////////////////////////////////////////////////////
// PLOT: vector of atoms
///////////////////////////////////////////////////////////////////////////

typedef int atom_tp;
typedef stapl::vector<atom_tp> vec_atom_tp;
typedef stapl::vector<atom_tp,
        stapl::view_based_partition<dist_spec_tp>,
        stapl::view_based_mapper<dist_spec_tp> > vec_atom_dist_tp;
...
bool run_atom_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  times[0] += time_delta;
  labels[0] = string("container: vector, construct, defaults");
  seconds(time_start);

  // METRIC: construct container with non-default initializer
  vec_atom_tp u_ct(size, 314159);

  seconds(time_end);
  time_delta = time_end - time_start;
  times[1] += time_delta;
  labels[1] = string("container: vector, construct, non-default initializer");
  seconds(time_start);

  // METRIC: construct container with non-default distribution
  dist_spec_tp old_dist = stapl::block(size,1000);
  vec_atom_dist_tp w_ct(old_dist);

  seconds(time_end);
  time_delta = time_end - time_start;
  times[2] += time_delta;
  labels[2] = string("container: vector, construct, non-default distribution");
  seconds(time_start);

  // METRIC: redistribute container
  dist_spec_tp new_dist = stapl::block_cyclic(size,100);
  w_ct.redistribute(new_dist);

  seconds(time_end);
  time_delta = time_end - time_start;
  times[3] += time_delta;
  labels[3] = string("container: vector, redistribute");
...
///////////////////////////////////////////////////////////////////////////
// PLOT: vector of STL vectors
///////////////////////////////////////////////////////////////////////////

typedef vector<atom_tp> stl_tp;
typedef stapl::vector<stl_tp> vec_stl_tp;
typedef stapl::vector<stl_tp,
        stapl::view_based_partition<dist_spec_tp>,
        stapl::view_based_mapper<dist_spec_tp> > vec_stl_dist_tp;
...
bool run_stl_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  // METRIC: construct container with non-default initializer
  vec_stl_tp u_ct(outer, vector<atom_tp>(inner,314159) );
...
///////////////////////////////////////////////////////////////////////////
// plot: vector of STAPL vectors
///////////////////////////////////////////////////////////////////////////

typedef stapl::vector<int> nest_tp;
typedef stapl::vector<nest_tp> vec_nest_tp;
typedef stapl::vector<nest_tp,
        stapl::view_based_partition<dist_spec_tp>,
        stapl::view_based_mapper<dist_spec_tp> > vec_nest_dist_tp;
