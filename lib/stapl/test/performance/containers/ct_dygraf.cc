...
///////////////////////////////////////////////////////////////////////////
// FEATURE: dynamic graph container
///////////////////////////////////////////////////////////////////////////
...
stapl::exit_code stapl_main(int argc, char **argv) {
  ...
  stapl::exit_code code1 = experiment(1, model, "dygraf_ct atom", 
                           atom_times, atom_labels);
  stapl::exit_code code2 = experiment(2, model, "dygraf_ct stl", 
                           stl_times, stl_labels);
#ifdef NESTED_ACTIVE
  stapl::exit_code code3 = experiment(3, model, "dygraf_ct nest", 
...
///////////////////////////////////////////////////////////////////////////
// PLOT: graph with atom properties
///////////////////////////////////////////////////////////////////////////
...
bool run_atom_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  srand(16807);
  dygraf_atom_tp g_ct(outer);

  seconds(time_end);
  time_delta = time_end - time_start;
  times[0] += time_delta;
  labels[0] = string("container: dynamic graph, construct, defaults");
  seconds(time_start);

  // METRIC: construct container with non-default distribution
  dist_spec_tp old_dist = stapl::block(outer,10);
  dygraf_atom_dist_tp h_ct(old_dist);

  seconds(time_end);
  time_delta = time_end - time_start;
  times[1] += time_delta;
  labels[1] = string("container: dynamic graph, construct, non-default distribution");
  seconds(time_start);

  // METRIC: add graph vertices with sequential loop
  for ( int i=0; i<20; i++ ) {
    g_ct.add_vertex(fibo[i]*10);
  }

  seconds(time_end);
  time_delta = time_end - time_start;
  times[2] += time_delta;
  labels[2] = string("container, dynamic graph, add vertices");
  seconds(time_start);
  ...
  for ( auto vtx_it = g_ct.begin(); vtx_it != g_ct.end(); ++vtx_it ) {
    for ( size_t j = 0; j < vert_cnt/2; j++ ) {
      ...
      auto ed = g_ct.add_edge( src, dest, prime_nums[k%init_count] );
  ...
  time_delta = time_end - time_start;
  times[3] += time_delta;
  labels[3] = string("container, dynamic graph, add edges");
  ...
  time_delta = time_end - time_start;
  times[4] += time_delta;
  labels[4] = string("container: dynamic graph, initialize");
  seconds(time_start);
#endif
...
///////////////////////////////////////////////////////////////////////////
// PLOT: graph with STL vector properties
///////////////////////////////////////////////////////////////////////////
...
bool run_stl_test(size_t outer, size_t inner, double *times, string *labels ) {
  ...
  // METRIC: construct container with non-default distribution
  dist_spec_tp old_dist = stapl::block(outer,10);
  dygraf_stl_dist_tp h_ct(old_dist);
  ...
  time_delta = time_end - time_start;
  times[2] += time_delta;
  labels[2] = string("container: dynamic graph, initialize");
  seconds(time_start);
#endif
  ...
  for ( int i=0; i<20; i++ ) {
    size_t size = base + ( rand() % inner );
    g_ct.add_vertex( stl_tp(size, fibo[i]) );
  }

  seconds(time_end);
  time_delta = time_end - time_start;
  times[3] += time_delta;
  labels[3] = string("container: dynamic graph, add vertices");
...
///////////////////////////////////////////////////////////////////////////
// plot: graph with STAPL vector properties
///////////////////////////////////////////////////////////////////////////
