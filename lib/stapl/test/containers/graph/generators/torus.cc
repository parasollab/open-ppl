...
void test_output(GraphView& v, size_t x, size_t y, std::string s)
{
  STAPL_TEST_REPORT(v.size() == x*y,
                    std::string("Torus Generator: Testing size  " + s));

  boost::unordered_map<size_t, size_t> distribution =
    degree_distribution(v);

  bool passed  = (distribution.size() == 1);
       passed &= (distribution[4] == x*y);

  STAPL_TEST_REPORT(passed, std::string("Torus Generator: Testing degree" + s));

  rmi_fence();
}

template<typename Graph>
void test_torus_gen(size_t x, size_t y, std::string s)
{
  ...
  view_type v = generators::make_torus<view_type>(x, y);
  v1 = generators::make_torus<view_type>(v1, x, y);
  v2 = generators::make_torus<view_type>(v2, x, y);
...
stapl::exit_code stapl_main(int argc, char* argv[])
{
  ...
  test_torus_gen<d_graph_type>(x, y, " [Directed Graph]   ");
  test_torus_gen<u_graph_type>(x, y, " [UnDirected Graph] ");
