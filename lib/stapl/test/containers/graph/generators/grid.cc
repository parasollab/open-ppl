...
void test_mesh_gen(size_t x, size_t y, std::string s)
{
  ...
  std::array<std::size_t, 2> dims{{x,y}};

  // generate vertices and edges in graph
  view_type v = generators::make_grid<view_type>(dims);
  v1 = generators::make_grid<view_type>(v1, dims);
  v2 = generators::make_grid<view_type>(v2, dims);
