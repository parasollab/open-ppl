...
void test_output(GraphView& v, size_t x, size_t y, std::string s)
{
  STAPL_TEST_REPORT(v.size() == x*y,
                    std::string("Lists-Tree Generator: Testing size  " + s));
  ...
  if (v.is_directed()) {
    ...
    passed &= (distribution[1] == y+1);
    passed &= (distribution[2] == x*y-(y+1));
  } else {
    passed = (distribution.size() == 3);
    passed &= (distribution[1] == y);
    passed &= (distribution[2] == x*y-y-(y-2));
    passed &= (distribution[3] == y-2);
  }

  STAPL_TEST_REPORT(passed, std::string("Lists-Tree Generator: Testing degree"
                                        + s));
...
void test_lists_tree_gen(size_t x, size_t y, std::string s)
{
  ...
  view_type v = generators::make_lists_tree<view_type>(x, y);
  v1 = generators::make_lists_tree<view_type>(v1, x, y);
  v2 = generators::make_lists_tree<view_type>(v2, x, y);
...
stapl::exit_code stapl_main(int argc, char* argv[])
{
  ...
  test_lists_tree_gen<d_graph_type>(x, y, " [Directed Graph]   ");
  test_lists_tree_gen<u_graph_type>(x, y, " [UnDirected Graph] ");
