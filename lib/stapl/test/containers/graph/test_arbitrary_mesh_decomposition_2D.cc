...
{
  if (argc < 4) {
    std::cerr << "usage: " << argv[0] << " filename2D num_partitions_x"
    " num_partitions_y" << std::endl;
    return EXIT_FAILURE;
  }

  typedef unstructured_mesh<2> mesh_type;
  typedef geom_vector<2, double> geom_vector_type;
  typedef graph_view<mesh_type> view_type;
  string filename(argv[1]);
  size_t num_x = atoi(argv[2]);
  size_t num_y = atoi(argv[3]);
  size_t num_part = num_x*num_y;

  mesh_type mesh = mesh_type(import_silo_file(filename));
  view_type mesh_vw(mesh);

  gpartition<view_type>
  gp = graph_partition(mesh_vw, arbitrary_mesh_collapser<2, 2, double>(
                       make_tuple(num_x, num_y),
                       make_tuple(geom_vector_type(0, 1),
                       geom_vector_type(1, 0))));
  ...
  generate_silo_file(mesh, partition_id_map, "part_mesh_2d");
  ...
  bool res = (gp.size()==num_part) && (res_num_vertices==mesh_vw.size())
                                   && empty_intersection;
  STAPL_TEST_REPORT(res,"Testing 2D unstructured mesh partitioner\t");
