                ...
                std::stringstream& o)
{
  ...
  typedef stapl::dynamic_graph<stapl::DIRECTED, stapl::NONMULTIEDGES,
            long int, int, vb_part, vb_map> inner_cont_t;
  typedef stapl::dynamic_graph<stapl::DIRECTED, stapl::NONMULTIEDGES,
...
stapl::exit_code stapl_main(int argc, char** argv)
{
  ...
  counter_t bal_arr;
  confidence_interval_controller bal_dyn_gph_ctrl(samples, samples, 0.05);

  // Controller for tests of view-based distributions
  // Only one controller is used for simplicity.
  counter_t vb_arr;
  confidence_interval_controller bal_vb_dyn_gph_ctrl(samples, samples, 0.05);
  ...
  bal_report << "Test : dynamic_graph_default_dist\n"
             << "Version : STAPL\n";
  bool bal_pass = true;
  ...
  bal_vb_report << "Test : dynamic_graph_viewbased_balanced_dist\n"
                << "Version : STAPL\n";
  ...
  blk_vb_report << "Test : dynamic_graph_viewbased_blocked_dist\n"
                << "Version : STAPL\n";
  ...
  blk_cyc_vb_report << "Test : dynamic_graph_viewbased_block_cyclic_dist\n"
                    << "Version : STAPL\n";
  ...
  arb_vb_report << "Test : dynamic_graph_viewbased_arbitrary_dist\n"
                 << "Version : STAPL\n";
  ...
  comp_vb_report << "Test : homogeneous_comp_dynamic_graph_dist\n"
                 << "Version : STAPL\n";
  bool comp_vb_pass = true;

  std::tuple<double, double, bool> alg_times;
  while (bal_dyn_gph_ctrl.iterate() || bal_vb_dyn_gph_ctrl.iterate())
  {
    // Test default distributed dynamic_graph
    bal_arr.reset();
    alg_times = run_balanced(n, bal_arr, bal_report);
    bal_dyn_gph_ctrl.push_back(bal_arr.value());
    ...
    bal_vb_dyn_gph_ctrl.push_back(vb_arr.value());
    ...
    { stapl::cyclic(n1), stapl::balance(n2),
      stapl::block_cyclic(n3, comp_blk_sz) };
    vb_arr.reset();
  ...
  // Output results for default balanced dynamic_graph
  if (bal_pass)
    bal_report << "Status : PASS\n";
  else
    bal_report << "Status : FAIL\n";
  bal_dyn_gph_ctrl.report(bal_report);

  std::stringstream bal_dyn_gph_gen;
  bal_dyn_gph_gen << "Test : generate_on_bal_dynamic_graph\n"
              << "Version : STAPL\n";
  if (bal_pass)
    bal_dyn_gph_gen << "Status : PASS\n";
  else
    bal_dyn_gph_gen << "Status : FAIL\n";
  report(bal_dyn_gph_gen, bal_gen_times);

  std::stringstream bal_dyn_gph_acc;
  bal_dyn_gph_acc << "Test : accumulate_on_bal_dynamic_graph\n"
              << "Version : STAPL\n";
  if (bal_pass)
    bal_dyn_gph_acc << "Status : PASS\n";
  else
    bal_dyn_gph_acc << "Status : FAIL\n";
  report(bal_dyn_gph_acc, bal_acc_times);

  stapl::do_once([&](void) {
    std::cerr << bal_report.str();
    std::cerr << bal_dyn_gph_gen.str();
    std::cerr << bal_dyn_gph_acc.str();
  });


  // Output results for view-based balanced dynamic_graph
  if (bal_vb_pass)
    bal_vb_report << "Status : PASS\n";
  else
    bal_vb_report << "Status : FAIL\n";
  bal_vb_dyn_gph_ctrl.report(bal_vb_report);

  std::stringstream bal_vb_dyn_gph_gen;
  bal_vb_dyn_gph_gen << "Test : generate_on_vb_bal_dynamic_graph\n"
              << "Version : STAPL\n";
  if (bal_vb_pass)
    bal_vb_dyn_gph_gen << "Status : PASS\n";
  else
    bal_vb_dyn_gph_gen << "Status : FAIL\n";
  report(bal_vb_dyn_gph_gen, bal_vb_gen_times);

  std::stringstream bal_vb_dyn_gph_acc;
  bal_vb_dyn_gph_acc << "Test : accumulate_on_vb_bal_dynamic_graph\n"
              << "Version : STAPL\n";
  if (bal_vb_pass)
    bal_vb_dyn_gph_acc << "Status : PASS\n";
  else
    bal_vb_dyn_gph_acc << "Status : FAIL\n";
  report(bal_vb_dyn_gph_acc, bal_vb_acc_times);

  stapl::do_once([&](void) {
    std::cerr << bal_vb_report.str();
    std::cerr << bal_vb_dyn_gph_gen.str();
    std::cerr << bal_vb_dyn_gph_acc.str();
  });

  // Output results for view-based block dynamic_graph
  std::stringstream blk_vb_dyn_gph_ctr;
  blk_vb_dyn_gph_ctr << "Test : construct_vb_blk_dynamic_graph\n"
              << "Version : STAPL\n";
  if (blk_vb_pass)
    blk_vb_dyn_gph_ctr << "Status : PASS\n";
  else
    blk_vb_dyn_gph_ctr << "Status : FAIL\n";
  report(blk_vb_dyn_gph_ctr, blk_vb_ctr_times);

  std::stringstream blk_vb_dyn_gph_gen;
  blk_vb_dyn_gph_gen << "Test : generate_on_vb_blk_dynamic_graph\n"
              << "Version : STAPL\n";
  if (blk_vb_pass)
    blk_vb_dyn_gph_gen << "Status : PASS\n";
  else
    blk_vb_dyn_gph_gen << "Status : FAIL\n";
  report(blk_vb_dyn_gph_gen, blk_vb_gen_times);

  std::stringstream blk_vb_dyn_gph_acc;
  blk_vb_dyn_gph_acc << "Test : accumulate_on_vb_blk_dynamic_graph\n"
              << "Version : STAPL\n";
  if (blk_vb_pass)
    blk_vb_dyn_gph_acc << "Status : PASS\n";
  else
    blk_vb_dyn_gph_acc << "Status : FAIL\n";
  report(blk_vb_dyn_gph_acc, blk_vb_acc_times);

  stapl::do_once([&](void) {
    std::cerr << blk_vb_dyn_gph_ctr.str();
    std::cerr << blk_vb_dyn_gph_gen.str();
    std::cerr << blk_vb_dyn_gph_acc.str();
  });

  // Output results for view-based block-cyclic dynamic_graph
  std::stringstream blk_cyc_vb_dyn_gph_ctr;
  blk_cyc_vb_dyn_gph_ctr << "Test : construct_vb_blk_cyc_dynamic_graph\n"
              << "Version : STAPL\n";
  if (blk_cyc_vb_pass)
    blk_cyc_vb_dyn_gph_ctr << "Status : PASS\n";
  else
    blk_cyc_vb_dyn_gph_ctr << "Status : FAIL\n";
  report(blk_cyc_vb_dyn_gph_ctr, blk_cyc_vb_ctr_times);

  std::stringstream blk_cyc_vb_dyn_gph_gen;
  blk_cyc_vb_dyn_gph_gen << "Test : generate_on_vb_blk_cyc_dynamic_graph\n"
              << "Version : STAPL\n";
  if (blk_cyc_vb_pass)
    blk_cyc_vb_dyn_gph_gen << "Status : PASS\n";
  else
    blk_cyc_vb_dyn_gph_gen << "Status : FAIL\n";
  report(blk_cyc_vb_dyn_gph_gen, blk_cyc_vb_gen_times);

  std::stringstream blk_cyc_vb_dyn_gph_acc;
  blk_cyc_vb_dyn_gph_acc << "Test : accumulate_on_vb_blk_cyc_dynamic_graph\n"
              << "Version : STAPL\n";
  if (blk_cyc_vb_pass)
    blk_cyc_vb_dyn_gph_acc << "Status : PASS\n";
  else
    blk_cyc_vb_dyn_gph_acc << "Status : FAIL\n";
  report(blk_cyc_vb_dyn_gph_acc, blk_cyc_vb_acc_times);

  stapl::do_once([&](void) {
    std::cerr << blk_cyc_vb_dyn_gph_ctr.str();
    std::cerr << blk_cyc_vb_dyn_gph_gen.str();
    std::cerr << blk_cyc_vb_dyn_gph_acc.str();
  });

  // Output results for view-based arbitrary graph
  std::stringstream arb_vb_dyn_gph_ctr;
  arb_vb_dyn_gph_ctr << "Test : construct_vb_arb_dynamic_graph\n"
                     << "Version : STAPL\n";
  if (arb_vb_pass)
    arb_vb_dyn_gph_ctr << "Status : PASS\n";
  else
    arb_vb_dyn_gph_ctr << "Status : FAIL\n";
  report(arb_vb_dyn_gph_ctr, arb_vb_ctr_times);

  std::stringstream arb_vb_dyn_gph_gen;
  arb_vb_dyn_gph_gen << "Test : generate_on_vb_arb_dynamic_graph\n"
                     << "Version : STAPL\n";
  if (arb_vb_pass)
    arb_vb_dyn_gph_gen << "Status : PASS\n";
  else
    arb_vb_dyn_gph_gen << "Status : FAIL\n";
  report(arb_vb_dyn_gph_gen, arb_vb_gen_times);

  std::stringstream arb_vb_dyn_gph_acc;
  arb_vb_dyn_gph_acc << "Test : accumulate_on_vb_arb_dynamic_graph\n"
                     << "Version : STAPL\n";
  if (arb_vb_pass)
    arb_vb_dyn_gph_acc << "Status : PASS\n";
  else
    arb_vb_dyn_gph_acc << "Status : FAIL\n";
  report(arb_vb_dyn_gph_acc, arb_vb_acc_times);

  stapl::do_once([&](void) {
    std::cerr << arb_vb_dyn_gph_ctr.str();
    std::cerr << arb_vb_dyn_gph_gen.str();
    std::cerr << arb_vb_dyn_gph_acc.str();
  });

  // Output results for composed view-based homogeneous-by-level dynamic_graph
  std::stringstream comp_cyc_bal_blk_cyc_vb_ctr;
  comp_cyc_bal_blk_cyc_vb_ctr << "Test : construct_comp_homo_vb_dynamic_graph\n"
                              << "Version : STAPL\n";
  if (comp_vb_pass)
  ...
  comp_vb_gen << "Test : generate_on_comp_homo_vb_dynamic_graph\n"
              << "Version : STAPL\n";
  if (comp_vb_pass)
  ...
  comp_vb_acc << "Test : accumulate_on_comp_homo_vb_dynamic_graph\n"
              << "Version : STAPL\n";
  if (comp_vb_pass)
