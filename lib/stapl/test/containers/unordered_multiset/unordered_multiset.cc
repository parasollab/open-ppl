...
void test_set(Set& s, size_t n)
{
  ...
  // tests the insert function by inserting n/p elements twice and then
  ...
  rmi_fence();
  STAPL_TEST_REPORT(s.size() == 2*n, "Testing insert     ");
  rmi_fence();
  ...
  rmi_fence();
  STAPL_TEST_REPORT(cleared_size == 0 && s.size() == 2*n,"Testing clear      ");
  rmi_fence();
  ...
  STAPL_TEST_REPORT(amount == n*(n-1), "Testing global iter");
 ...
 // found). Also looks for an element beyond end, which should return end()
  ...
  rmi_fence();
  STAPL_TEST_REPORT(s.size() == 0 && erase_count == 2*element_count,
                                                  "Testing erase_sync ");
  rmi_fence();
}
...
stapl::exit_code stapl_main(int argc, char* argv[])
{
  ...
  typedef stapl::unordered_multiset<key_type> set_type;
  ...
  stapl::unordered_map_partition<Hasher,Dom>
    mypart = stapl::unordered_map_partition<Hasher,Dom>();
