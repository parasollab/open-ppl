...
#ifndef STAPL_CONTAINERS_SEQUENTIAL_GRAPH_VDG_STORAGE_LIST_HPP
#define STAPL_CONTAINERS_SEQUENTIAL_GRAPH_VDG_STORAGE_LIST_HPP
...
/// @todo Derive this from @ref vertex_descriptor_vector to remove redundant
/// code.
//////////////////////////////////////////////////////////////////////
template <graph_attributes D, class VD_Base, class VertexP, class EdgeP>
class vertex_descriptor_list
{
private:
  typedef vertex_descriptor_list<D, VD_Base, VertexP, EdgeP> this_type;
  ...
  typedef std::list<vertex_impl_type> vertex_set_type;
...
public:
  vertex_descriptor_list()
    : m_version(-1)
  {
  }

  vertex_descriptor_list(VD_Base const& vd)
    : m_user_descriptor(vd), m_version(-1)
  {
  }

  vertex_descriptor_list(VD_Base const& vd, vertex_iterator const& vi,
                         long const& version)
  ...
  {
    m_user_descriptor += other.m_user_descriptor;
    m_version = -1;
    return *this;
  }
...
/// descriptors for the smart list storage, and tracks the version
...
class vdg_base_list
{
  typedef vdg_base_list<VD> this_type;
protected:
  size_t m_curr_vd;
...
//////////////////////////////////////////////////////////////////////
/// @brief An adjacency list using an std::list for storing vertices.
...
/// smart descriptors that can provide faster vertex-lookups if they are
/// reused often.
//////////////////////////////////////////////////////////////////////
template <class Traits>
class vdg_storage_list
{
  typedef vdg_storage_list<Traits> this_type;
...
public:
  ...
  typedef vdg_base_list<vertex_descriptor> vdg_type;

  /// The vertices are stored in an std::list.
  typedef std::list<vertex_impl_type> vertex_set_type;
...
public:
  ...
  //////////////////////////////////////////////////////////////////////
  vdg_storage_list(size_t nv = 0)
    : m_vdg(0)
  {
    // m_storage.resize(_nv);
  ...
  vdg_storage_list(this_type const& other)
    : m_vdg(other.m_vdg)
  {
  ...
  //////////////////////////////////////////////////////////////////////
  inline vertex_descriptor add_vertex(vertex_descriptor& vd)
  {
  ...
  /// @return The descriptor of the added vertex.
  //////////////////////////////////////////////////////////////////////
  inline vertex_descriptor add_vertex(vertex_property const& vp)
  {
                               ...
                               vertex_property const& vp)
  {
    ...
    m_vdg.update(vd);
    return vertex_descriptor(v.descriptor(),
                             m_storage.insert(m_storage.end(), v), 0);
  }
  ...
  bool delete_vertex(vertex_descriptor const& vd, iterator const& vi)
  {
    ...
    //invalidate the vertex descriptor
    vertex_descriptor& vd_temp = const_cast<vertex_descriptor&>(vd);
    vd_temp.vi(m_storage.end());
    vd_temp.version(-1);
    return true;
  }
  ...
  iterator find_vertex(vertex_descriptor const& vd_input)
  {
    ...
    if (this->size() == 0)
      return this->end();
    // look forward from m_storage.begin().
    vi = this->begin();
    while (vi != this->end()) {
      if ((*vi).descriptor() == vd) {
