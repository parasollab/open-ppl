...
stapl::exit_code stapl_main(int argc, char **argv) {
  ...
  // apply algorithm
  int ndx = -1;
  vec_int_vw_tp::reference ref = stapl::min_element(v_vw);
  if( !stapl::is_null_reference(ref) ) {
    ndx = stapl::index_of(ref);
  }

  int ndx_c = -1;
  vec_int_vw_tp::reference ref_c = stapl::min_element(w_vw, less_fn());
  ...
  zout.open("refman_minelement.txt");
  stapl::do_once( msg_val<val_tp>( zout, "ndx ", ndx ) );
  stapl::do_once( msg_val<val_tp>( zout, "ndx_c ", ndx_c ) );
