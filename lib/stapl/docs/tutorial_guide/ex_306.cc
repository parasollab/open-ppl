...
stapl::exit_code stapl_main(int argc, char **argv) {
  ...
  zout.open("ex_306.out");

  size_t size = 1;
  stapl::do_once( msg( zout, "Example 306" ) );
  int result = ex_306(size, zin, zout);
  stapl::do_once( msg_val<int>( zout, "Result ", result ) );
  return EXIT_SUCCESS;
}

struct ex_306_gen_wf {
private:
  size_t m_card;
public:
  ex_306_gen_wf(size_t c)
    : m_card(c)
  { }
...
struct ex_306_fill_wf {
  typedef void result_type;
  template <typename View1, typename Elem>
  result_type operator()(View1 const &vw1, Elem count) {
    stapl::do_once(ex_306_gen_wf(count), vw1 ); // ## 4
  }
};

struct ex_306_process_wf {
...
struct ex_306_show_wf {
private:
  stapl::stream<ofstream> m_zout;
public:
  ex_306_show_wf(stapl::stream<ofstream> const& zout)
    : m_zout(zout)
  { }
...
size_t ex_306( size_t model,
  ...
  stapl::map_func( ex_306_fill_wf(), a_vw, len_vw ); // ## 10

  int res = stapl::map_reduce(ex_306_process_wf(), max_int_wf(), // ## 11
                              a_vw );

  stapl::do_once( msg( zout, "a:") );

  stapl::map_func(ex_306_show_wf(zout), a_vw ); // ## 12
