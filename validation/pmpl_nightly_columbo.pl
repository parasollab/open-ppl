...
$ADMIN_MAILTO = "sjacobs\@cse.tamu.edu";
$workdir  = "/scratch/columbo/sjacobs/pmpl_nightly";
...
if (!defined $opt_r || !(($opt_r eq "manifold") || ($opt_r eq "surface") || ($opt_r eq "reach"))) { 
  die "must define robot type (-r manifold | surface | reach)"; 
}
...
$PLATFORM = $opt_c;
$ROBOT = "-DPMPManifold";
if($opt_r eq "surface") { $ROBOT = "-DPMPCfgSurface"; }
...
# 
if ($opt_c eq "LINUX_gcc" && $opt_p eq "1") {
...
$OUTPUT = $OUTPUT.`module load boost-1.48`;
...
print MAIL "Subject: PMPL nightly - columbo.cse.tamu.edu pmpl.$opt_c.$opt_r.debug$opt_d.parallel$opt_p\n";
