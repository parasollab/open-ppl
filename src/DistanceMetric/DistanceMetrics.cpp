...
ParseXML(MPProblem* in_pProblem, XMLNodeReader::childiterator citr)
{
  if(citr->getName() == "euclidean") {
    par_label = citr->stringXMLParameter("Label",false,"","Label");
    DistanceMetricMethod* df = new EuclideanDistance(par_label);
    AddDMMethod(df->GetObjectLabel(),DistanceMetricPointer(df));
    citr->warnUnrequestedAttributes();

  } else if(citr->getName() == "scaledEuclidean") {
    par_label = citr->stringXMLParameter("Label",false,"","Label");
    double par_scale = citr->numberXMLParameter("scale",false,double(0.5),double(0.0),double(1.0),"Scale Factor");
    DistanceMetricMethod* df = new ScaledEuclideanDistance(par_label, par_scale);
    AddDMMethod(df->GetObjectLabel(),DistanceMetricPointer(df));
    citr->warnUnrequestedAttributes();

  } else if( citr->getName() == "knotTheory") {
    par_label = citr->stringXMLParameter("Label",false,"","Label");
    DistanceMetricMethod* df = new KnotTheoryDistance(par_label);
    AddDMMethod(df->GetObjectLabel(),DistanceMetricPointer(df));
    citr->warnUnrequestedAttributes();
    ...
    par_label =citr->stringXMLParameter("Label",false,"","Label");
    par_use_rotational = citr->numberXMLParameter("use_rotational",false,int(0),int(0),int(1),"Rotational distances used");
    DistanceMetricMethod* df = new UniformEuclideanDistance(par_label,par_use_rotational);
    AddDMMethod(df->GetObjectLabel(),DistanceMetricPointer(df));
    citr->warnUnrequestedAttributes();

  } else if(citr->getName() == "pureEuclidean") {
    par_label = citr->stringXMLParameter("Label",false,"","Label");
    DistanceMetricMethod* df = new PureEuclideanDistance(par_label);
    AddDMMethod(df->GetObjectLabel(),DistanceMetricPointer(df));
    citr->warnUnrequestedAttributes();

  } else if(citr->getName() == "rmsd") {
    par_label = citr->stringXMLParameter("Label",false,"","Label");
    DistanceMetricMethod* df = new RmsdDistance(par_label);
    AddDMMethod(df->GetObjectLabel(),DistanceMetricPointer(df));
    citr->warnUnrequestedAttributes();
    ...
    DistanceMetricMethod* df = new LPSweptDistance(par_label,lp->selected[0]->CreateCopy(), pos_res, ori_res, use_bbox);
    ...
    DistanceMetricMethod* df = new BinaryLPSweptDistance(par_label,lp->selected[0]->CreateCopy(), pos_res, ori_res, tolerance, max_attempts, use_bbox);
    ...
    par_label = citr->stringXMLParameter("Label",false,"","Label");
    DistanceMetricMethod* df = new ReachableDistance(par_label);
    AddDMMethod(df->GetObjectLabel(),DistanceMetricPointer(df));
    citr->warnUnrequestedAttributes();
 #else
...
{
  DistanceMetricPointer to_return = DistanceMetricContainer::GetMethod(in_strLabel);
  if(to_return.get() == NULL) 
  {
    cerr << "\n\n\tERROR in GetDMMethod, requesting dm method labeled \"" << in_strLabel << "\" which does not exist in the list:\n";
    for(map<string, shared_ptr<DistanceMetricMethod> >::const_iterator M = methods.begin(); M != methods.end(); ++M)
...
AddDMMethod(string in_strLabel, DistanceMetricPointer in_ptr) 
{
  ...
  DistanceMetricContainer::AddMethod(in_strLabel, in_ptr);
   /*  if(in_strLabel =="default"){
   
        
    cerr<<"\n\n\t Error , More than one default label specified \""<<endl;
     cerr<<"exiting...\n";
      exit(-1);
...
DistanceMetricMethod(std::string in_strLabel) :LabeledObject(in_strLabel) {
}
...
EuclideanDistance::
EuclideanDistance(string in_strLabel) : DistanceMetricMethod(in_strLabel) {
  type = CS;
}
...
ScaledEuclideanDistance(string in_strLabel) : EuclideanDistance(in_strLabel) {
  sValue = 0.5;
}


ScaledEuclideanDistance::
ScaledEuclideanDistance(string in_strLabel, double _sValue) : EuclideanDistance(in_strLabel) {
...
KnotTheoryDistance::
KnotTheoryDistance(string in_strLabel): EuclideanDistance(in_strLabel){
}
...
UniformEuclideanDistance(string in_strLabel) : DistanceMetricMethod(in_strLabel) {
  type = CS;
  useRotational = false;
}
...
UniformEuclideanDistance::
UniformEuclideanDistance(string in_strLabel, int _useRotational) : DistanceMetricMethod(in_strLabel) {
...
PureEuclideanDistance(string in_strLabel) : DistanceMetricMethod(in_strLabel) {
  type = CS;
}
...
MinkowskiDistance::
MinkowskiDistance(string in_strLabel) : DistanceMetricMethod(in_strLabel) {
  type = CS;
  r1 = 3;
  r2 = 3;
  r3 = 1.0/3;
...
ManhattanDistance::
ManhattanDistance(string in_strLabel) : DistanceMetricMethod(in_strLabel) {
  type = CS;
}
...
CenterOfMassDistance(string in_strLabel) : DistanceMetricMethod(in_strLabel) {
  type = WS;
}
...
RmsdDistance::
RmsdDistance(string in_strLabel) : EuclideanDistance(in_strLabel) {
}
...
LPSweptDistance::
LPSweptDistance(string in_strLabel) : DistanceMetricMethod(in_strLabel) {
}

LPSweptDistance::
LPSweptDistance(string in_strLabel, LocalPlannerMethod<CfgType, WeightType>* _lp_method) : DistanceMetricMethod(in_strLabel), lp_method(_lp_method), positionRes(0.1), orientationRes(0.1), use_bbox(false) {
}

LPSweptDistance::
LPSweptDistance(string in_strLabel, LocalPlannerMethod<CfgType, WeightType>* _lp_method, double pos_res, double ori_res, bool bbox) : DistanceMetricMethod(in_strLabel), lp_method(_lp_method), positionRes(pos_res), orientationRes(ori_res), use_bbox(bbox) {
}
...
BinaryLPSweptDistance(string in_strLabel) : DistanceMetricMethod(in_strLabel) {
}

BinaryLPSweptDistance::
BinaryLPSweptDistance(string in_strLabel, LocalPlannerMethod<CfgType, WeightType>* _lp_method) : DistanceMetricMethod(in_strLabel), lp_method(_lp_method), positionRes(0.1), orientationRes(0.1), tolerance(0.01), dist_calls_count(0), use_bbox(false) {
}

BinaryLPSweptDistance::
BinaryLPSweptDistance(string in_strLabel, LocalPlannerMethod<CfgType, WeightType>* _lp_method, double pos_res, double ori_res, double tolerance, int max_attempts, bool bbox) : DistanceMetricMethod(in_strLabel), lp_method(_lp_method), positionRes(pos_res), orientationRes(ori_res), tolerance(tolerance), max_attempts(max_attempts), dist_calls_count(0), use_bbox(bbox) {
}
