...
RSMDDistance::RSMDDistance() : EuclideanDistance() {
  m_name = "rmsd";
}

RSMDDistance::
RSMDDistance(XMLNodeReader& _node, MPProblem* _problem, bool _warn) : EuclideanDistance(_node, _problem, _warn) {
  m_name = "rmsd";
}

RSMDDistance::~RSMDDistance() {
}

vector<Vector3D> RSMDDistance::GetCoordinatesForRMSD(const Cfg& _c, Environment* _env) {
...
double RSMDDistance::Distance(Environment* _env, const Cfg& _c1, const Cfg& _c2) {
  vector<Vector3D> x = GetCoordinatesForRMSD(_c1, _env);
  vector<Vector3D> y = GetCoordinatesForRMSD(_c2, _env);
  return RMSD(x,y,x.size());
}

double RSMDDistance::RMSD(vector<Vector3D> _x, vector<Vector3D> _y, int _dim) {
