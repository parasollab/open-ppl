...
operator()(int in_RegionID) {
   ...
   while(!IsFinished())
      for(I itr = m_vecStrNodeGenLabels.begin(); itr != m_vecStrNodeGenLabels.end(); ++itr){ //For each node generation method mentioned.
         ...
         cout<<"VectorCfgs.size()::"<<vectorCfgs.size()<<endl;
         ...
         for(vector<CfgType>::iterator itr_cfg = vectorCfgs.begin();
            ...
            cout<<"iterating through new nodes"<<endl;
            if((*itr_cfg).IsLabel("VALID")) {  
               if((*itr_cfg).GetLabel("VALID")) {//Add to Free roadmap
                  ...
                  int newVID = region->roadmap.m_pRoadmap->AddVertex(*itr_cfg);
                  ...
                  for(J itr = m_vecStrNodeConnectionLabels.begin(); itr != m_vecStrNodeConnectionLabels.end(); ++itr){
                     ConnectMap<CfgType,WeightType>::NodeConnectionPointer
                     ...
                     connectmap->ConnectNodes(shared_ptr<NodeConnectionMethod<CfgType,WeightType> > (pConnection), region->GetRoadmap(), *pStatClass, 
                  ...
                  //-------------------//
                  cmap.reset();
                  int nNumCurCCs = get_cc_count(*(region->roadmap.m_pRoadmap),cmap);
                  ...
                  int n_num_nodes_tmp = region->roadmap.m_pRoadmap->get_num_vertices();
                  ...
                  char_ofstream << region->roadmap.m_pRoadmap->get_num_vertices() << ":" << stat_cccreate 
                                << ":" << stat_ccmerge 
...
PRMIncrementalStrategy::
IsFinished() {
   cout<<"m_total_samples:"<<m_totalSamples<<endl;
   cout<<"m_query_solved:"<<m_query_solved<<endl;
   if(m_query_solved || m_totalSamples>=num_nodes) {return true;}
   else {return false;}
...
public:
   ...
   bool operator()(VID _v1, VID _v2) { return is_same_cc(*(m_graph),cmap, _v1,_v2); }
...
cc_max_length(RoadmapGraph<CfgType,WeightType>* pGraph, VID _cc) {
   ...
   stapl::vector_property_map< stapl::stapl_color<size_t> > cmap;
   vector<VID> cc_vids;
   get_cc( *pGraph, cmap, _cc, cc_vids);
   double return_val = 0.0;
   for(int i=0; i<cc_vids.size(); ++i) {
...
merge_node_stats(MPRegion<CfgType,WeightType>* inout_pRegion, VID in_vid, Stat_Class& Stats) {
   ...
   pGraph->get_successors(in_vid,successors);
   typedef RoadmapGraph<CfgType, WeightType>::edge_descriptor ED;
   typedef RoadmapGraph<CfgType, WeightType>::vertex_iterator VI;
   typedef RoadmapGraph<CfgType, WeightType>::adj_edge_iterator EI;
   ...
   for(int i=0; i<successors.size(); ++i) {
      ...
      pGraph->delete_edge(in_vid,VID(successors[i]));
      pGraph->delete_edge(VID(successors[i]),in_vid);
   }
