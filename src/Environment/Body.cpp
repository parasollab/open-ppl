...
ComputeConvexHull() {
  ...
  typedef Polyhedron3::Halfedge_around_facet_circulator     HalfedgeFacetCirculator;
  ...
  //iterate through convex hull vertices
  ...
  //iterate through convex hull facets
    for(Polyhedron3::Facet_iterator fit = poly.facets_begin();
        fit != poly.facets_end(); ++fit) {
      m_convexHull.m_polygonList.push_back(GMSPolygon());
      GMSPolygon* lastPoly = &m_convexHull.m_polygonList.back();
      HalfedgeFacetCirculator he = fit->facet_begin();
      do
      {
        lastPoly->m_vertexList.push_back(std::distance(poly.vertices_begin(),
              he->vertex()));
      }
