...
#ifndef CRETRACTIONPATHMODIFIER_H_
#define CRETRACTIONPATHMODIFIER_H_
...
class CRetractionPathModifier : public PathModifierMethod<MPTraits> {
  ...
  private:
    void ValidatePath(vector<CfgType>& _path, vector<CfgType>& _path1, vector<CfgType>& _path2);
...
// Non-XML Constructor
template<class MPTraits>
CRetractionPathModifier<MPTraits>::CRetractionPathModifier(size_t _iter,
...
// XML Constructor
template<class MPTraits>
CRetractionPathModifier<MPTraits>::CRetractionPathModifier(MPProblemType* _problem, XMLNodeReader& _node) :
  PathModifierMethod<MPTraits>(_problem, _node), m_clearanceUtility(_problem, _node) {
    this->SetName("CRetractionPathModifier");
    m_iter = _node.numberXMLParameter("iter", true, 10, 0, MAX_INT, "Loop iterations");
    m_maxIter = _node.numberXMLParameter("maxIter", false, 1000, 0, MAX_INT, "Max loop iterations");
  }

template<class MPTraits>
void
CRetractionPathModifier<MPTraits>::PrintOptions(ostream& _os) const {
...
template<class MPTraits>
bool
CRetractionPathModifier<MPTraits>::ModifyImpl(vector<CfgType>& _path, vector<CfgType>& _newPath) {
  //C-Retraction
  //Input: _path
  //Output: _newPath
  //
  ...
  DistanceMetricPointer dm = this->GetMPProblem()->GetDistanceMetric(m_clearanceUtility.GetDistanceMetricLabel());
  ...
  while(i < m_maxIter && (avgs.size() < m_iter || fabs(avgs.front() - avgs.back()) > step/10)) {
    ...
    for(CIT cit = p2.begin()+1; cit+1 != p2.end(); ++cit) {
      ...
      if(ci1) {
        bool ci2 = m_clearanceUtility.CollisionInfo(*cit, clr2, boundary, cdInfo2);
...
void
CRetractionPathModifier<MPTraits>::ValidatePath(
    vector<CfgType>& _path, vector<CfgType>& _path1, vector<CfgType>& _path2) {
  //ValidatePath
  //Input: path, path'
  //Output: path''
  //
