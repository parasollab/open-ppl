...
ICreateChildAgent::
ICreateChildAgent(Robot* const _r) : Agent(_r) { 
}

ICreateChildAgent::
~ICreateChildAgent() {
  ICreateChildAgent::Uninitialize();
}

void
ICreateChildAgent::
InitializeMpSolution(MPSolution* _m) {
  m_solution = new MPSolution(_m->GetRobot());
  m_solution->SetRoadmap(_m->GetRoadmap());
}
...
GetNewTask() {
  ...
  auto parent = m_solution->GetRobot();
  auto task = new MPTask(parent);
  auto currentPos = m_robot->GetDynamicsModel()->GetSimulatedState();
  cout << "Parent label " << parent->GetLabel() << endl;
  cout << m_robot->GetLabel() << " this the current position " << currentPos << endl;
  //auto newGoal = GetRandomRoadmapPoint();
  //use the parent robot
  auto start = new CSpaceConstraint(parent, currentPos);
  //use the parent robot
  auto goal = new CSpaceConstraint(parent, "14 7 0 0 0 0");
...
Initialize() {
  ...
  SetTask(GetNewTask());
  //m_task = problem->GetTasks(m_solution->GetRobot()).front();
  
  // Use the planning library to find a path.
  m_library->Solve(problem, m_task, m_solution);

  // If th`e path is empty, PMPL has failed somehow.
