...
MoveBase(const Cfg& _goal, double _dt) {
  if(ros::ok()) {
    ...
    auto bestControl = robot->GetController()->operator()(current, _goal, 1.0);
    auto control = robot->GetSimulationModel()->Execute(bestControl);
...
GetCurrentState() {
  ...
  auto sharedMsg = ros::topic::waitForMessage<nav_msgs::Odometry>("/odometry/filtered");

  if(sharedMsg != NULL)
    msg = *sharedMsg;

  // ROS_INFO_STREAM(m_agent->GetRobot()->GetLabel()  << " Received: " << msg.pose);
