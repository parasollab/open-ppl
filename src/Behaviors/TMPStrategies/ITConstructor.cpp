...
ConstructIT(InteractionTemplate* _it){
  ...
  for(auto task : handoffTasks){
    ...
    Roadmap<MPTraits<Cfg>> testRm(m_superRobot);
    testRm.SetGraph(handoffSolution->GetRoadmap()->GetGraph());
    testRm.Write("indHandoffTemplate" + std::to_string(check) + ".map", m_problemCopy->GetEnvironment());
    testRm.SetGraph(nullptr);
    check++;

    // Store the roadmap for each task in the handoff
    _it->AddRoadmapGraph(handoffSolution->GetRoadmap()->GetGraph());
  ...
  Roadmap<MPTraits<Cfg>> testRm(m_superRobot);
  testRm.SetGraph(_it->GetConnectedRoadmap());
  testRm.Write("handoffTemplate.map", m_problemCopy->GetEnvironment());
  testRm.SetGraph(nullptr);
