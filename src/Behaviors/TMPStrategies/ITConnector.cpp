...
ConnectTemplates(RoadmapGraph<Cfg,DefaultWeight<Cfg>>* _graph){
  ...
  solution->GetRoadmap(robot)->SetGraph(_graph);
  ...
  *copyGraph = *_graph;
  ...
  testRm.Write("ConnectingTemplates-"+robot->GetCapability()+".map",
  ...
  testRm.Write("ConnectedTemplates-"+robot->GetCapability()+".map", robot->GetMPProblem()->GetEnvironment());
}
