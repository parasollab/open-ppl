...
RMSDDistance::
RMSDDistance() : DistanceMetricMethod() {
  this->SetName("RMSD");
}


RMSDDistance::
RMSDDistance(XMLNode& _node) : DistanceMetricMethod(_node) {
  this->SetName("RMSD");
}
...
std::vector<Vector3d>
RMSDDistance::
GetCoordinatesForRMSD(const Cfg& _c) {
  _c.ConfigureRobot();
...
double
RMSDDistance::
RMSD(std::vector<Vector3d> _x, std::vector<Vector3d> _y, int _dim) {
