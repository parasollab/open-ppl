...
AdaptiveRRT(double _wallPenalty, double _gamma, const GrowthSets& _growthSets,
    CostMethod _c) : BasicRRTStrategy(),
...
AdaptiveRRT::
AdaptiveRRT(XMLNode& _node) : BasicRRTStrategy(_node){
  this->SetName("AdaptiveRRT");
...
typename AdaptiveRRT::VID
AdaptiveRRT::
ExpandTree(Cfg& _dir){
  // Setup MP Variables
  //TODO: auto dm = this->GetDistanceMetric(this->m_dmLabel);
  auto dm = this->GetMPLibrary()->GetDistanceMetric(this->m_goalDmLabel);
  ...
  */
  Cfg& nearest = this->GetRoadmap()->GetVertex(kClosest[0].source);
  ...
  double minDist = this->GetMPLibrary()->GetExtender(this->m_exLabel)->GetMinDistance();
  ...
  LPOutput lpOutput;
  auto e = this->GetMPLibrary()->GetExtender(gm);
...
std::string
AdaptiveRRT::
SelectGrowthMethod(GrowthSet& _gs){
  if(this->m_debug)
...
void
AdaptiveRRT::
UpdateCost(double _cost, std::string _s, GrowthSet& _gs){
...
void
AdaptiveRRT::
RewardGrowthMethod(double _r, std::string _s, GrowthSet& _gs){
  if(this->m_debug)
...
typename AdaptiveRRT::VID
AdaptiveRRT::
UpdateTree(VID _expandNode, Cfg& _newCfg,
    Cfg& _dir, double _delta){
  auto dm = this->GetMPLibrary()->GetDistanceMetric(this->m_goalDmLabel);

  Cfg& nearest = this->GetRoadmap()->GetVertex(_expandNode);
...
typename AdaptiveRRT::VID
AdaptiveRRT::
AddNode(Cfg& _newCfg, VID _nearVID,
  ...
  Cfg& parentcfg = rdmp->GetVertex(_nearVID);
  if(this->m_debug)
  ...
  Cfg& newcfg = rdmp->GetVertex(newVID);
...
void
AdaptiveRRT::
AvgVisibility(Cfg& _cfg, double _val){
...
double
AdaptiveRRT::
CostInsensitiveProb(std::string _s, GrowthSet& _gs){
...
double
AdaptiveRRT::
GetCost(std::string _s, GrowthSet& _gs){
