#ifndef PPL_HIERARCHICAL_AS_RRT_H_
...
class HASRRT : public BasicRRTStrategy {
  public:
    ...
    struct SamplingRegion {
      ...
      size_t frontLineIndex{0};          ///< Which current index has valid samples?
      double attempts{1};                ///< Number of attempts to extend into this region.
      double successes{1};               ///< Number of successful attempts.
      vector<VID> samples;                 ///< Set of samples generated by the region traversing one edge
      int previousRegionIndex{-1};
      ///@}
      ...
      /// Push this region to the last edge intermediate point
      void PushToEnd() noexcept {
        edgeIndex = (*edgeIterator).property().size() - 1;
      }
    ...
    HASRRT(XMLNode& _node);

    virtual ~HASRRT() = default;
  ...
  protected:
    ...
    size_t GetClosestIntermediate(const Cfg& _cfg, SamplingRegion& _region);
    size_t GetBinaryIntermediate(const Cfg& _cfg, SamplingRegion& _region, bool _isQnear = false);
    ...
    /// The set of inactive dynamic sampling regions and associated metadata.
    std::vector<SamplingRegion> m_parentRegions;
    ...
    size_t m_selectedRegionIndex{0};

    bool m_selectedWholeEnv;
