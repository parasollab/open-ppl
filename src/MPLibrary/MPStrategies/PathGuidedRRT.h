...
SelectTarget() {
  ...
  // With probability 1 - m_bias or if moved through all waypoints, take random sample
  if(path.waypoints.size() == path.currentIndex or DRand() > m_bias) {
...
ExpandTree(const CfgType& _target) {
  ...
  auto& path = m_guidingPaths[m_guidingPathIndex];
  if(path.waypoints.size() == path.currentIndex)
    return vid;

  // Otherwise, check if new vertex reaches waypoint
  if(ReachedWaypoint(vid)) {
...
ReachedWaypoint(VID _vid) {
  ...
  auto& path = m_guidingPaths[m_guidingPathIndex];
  auto waypoint = path.waypoints[path.currentIndex]; 
  auto cfg = this->GetRoadmap()->GetVertex(_vid);
