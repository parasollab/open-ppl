...
class GroupRRTStrategy : public GroupStrategyMethod<MPTraits> {
  ...
  protected:
    ...
    bool m_restrictGrowth{false}; ///< Only from from nodes we generated?
    ...
    std::unordered_set<VID> m_vids; ///< VIDs created by this strategy
...
GroupRRTStrategy(XMLNode& _node) : GroupStrategyMethod<MPTraits>(_node) {
  ...
  m_restrictGrowth = _node.Read("restrictGrowth", false, false, 
      "Only expand from nodes generated by this strategy?");
...
Initialize() {
  ...
  if(!noStart and m_restrictGrowth)
    m_vids.insert(start);
