...
GroupDecoupledStrategy() : GroupStrategyMethod() {
  this->SetName("GroupDecoupledStrategy");
}


GroupDecoupledStrategy::
GroupDecoupledStrategy(XMLNode& _node) : GroupStrategyMethod(_node) {
  this->SetName("GroupDecoupledStrategy");
...
Iterate() {
  ...
  for(auto& task : *groupTask) {
    ...
    auto s = this->GetMPLibrary()->GetMPStrategy(strategy);
