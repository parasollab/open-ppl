...
GetSkeleton() {
  ...
  if(m_params.ioType != "read") {
    WorkspaceSkeleton* g = new WorkspaceSkeleton();
    ...
    for(auto pline : plines){
      ...
      for(size_t i = 0; i < 2; ++i) {
        ...
        if(findPt == vertexMap.end()) {
          ...
          vd[i] = g->AddVertex(ToPMPLPoint3d(Point(epts[i])));
      ...
      auto ed = g->AddEdge(vd[0], vd[1], intermediates);
...
GetMesoSkeleton() {
  ...
  if(m_params.ioType != "read") {
    ...
    BOOST_FOREACH(VD vd, vertices(m_meso)){
      auto svd = g->AddVertex(ToPMPLPoint3d(vd->point()));
    ...
    BOOST_FOREACH(ED ed, edges(m_meso)) {
      ...
      auto sed = g->AddEdge(vd[0], vd[1], intermediates);
      // Store the witnesses of the edge points
