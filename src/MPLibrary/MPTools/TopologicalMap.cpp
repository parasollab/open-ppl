...
TopologicalMap::
TopologicalMap() : MPBaseObject("TopologicalMap") {
  this->SetName("TopologicalMap");
}


TopologicalMap::
TopologicalMap(XMLNode& _node) : MPBaseObject(_node) {
  this->SetName("TopologicalMap");
...
const typename MPBaseObject::RoadmapType::VertexSet*
TopologicalMap::
...
const WorkspaceRegion*
TopologicalMap::
LocateRegion(const Cfg& _cfg, const size_t _bodyIndex) const {
  _cfg.ConfigureRobot();
...
LocateRegion(const Point3d& _point) const {
  ...
  if(this->m_debug) {
    ...
    auto vc = static_cast<CDType*>(this->GetMPLibrary()->GetValidityChecker(m_pqpLabel));
...
const WorkspaceRegion*
TopologicalMap::
LocateNearestRegion(const Cfg& _c, const size_t _bodyIndex) const {
  _c.ConfigureRobot();
...
size_t
TopologicalMap::
LocateCell(const Cfg& _cfg, const size_t _bodyIndex) const {
  _cfg.ConfigureRobot();
  auto body = _cfg.GetMultiBody()->GetBody(_bodyIndex);
...
typename TopologicalMap::NeighborhoodKey
TopologicalMap::
...
typename TopologicalMap::NeighborhoodKey
TopologicalMap::
LocateNeighborhood(const Cfg& _c) const {
  MethodTimer mt(this->GetStatClass(),
...
GetDecomposition() const {
  return this->GetMPLibrary()->GetMPTools()->GetDecomposition(m_decompositionLabel);
}
...
const typename TopologicalMap::OccupancyMap&
TopologicalMap::
...
const typename TopologicalMap::NeighborhoodKey&
TopologicalMap::
