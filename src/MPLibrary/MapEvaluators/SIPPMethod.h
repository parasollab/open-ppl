...
operator()() {
  ...
  for(; m_goalIndex < numGoals; ++m_goalIndex) {
    ...
    state.intervals = m_roadmap_intervals[start];
...
GeneratePath(const size_t _start, const size_t _goal) {
  ...
  while(current != _start) {
    ...
    m_wait_timesteps.push_back(ceil(m_transitionWait[state.vd][prev.vd]));
    prev = state;
    path.push_back(state.vd);
  }
...
DynamicPathWeight(typename SIPPGraph::adj_edge_iterator& _ei,
  ...
  if(!(siTool->ContainsTimestep(vertexIntervals, newDistance))) {
    ...
    if(interval.max - interval.min >= 0) {
      ...
      // in current interval
      Range<double> workingInterval = Range<double>(_sourceDistance,interval.max);
      ...
      Range<double> overlap = IntervalOverlaps(vertexIntervals, workingInterval);

      if(overlap.max - overlap.min >= 0) {
        waitTime = (overlap.min - edgeWeight) -_sourceDistance;
      } else {
  ...
  std::vector<Range<double>> overlappingIntervals = OverlappingIntervals(sourceIntervals, edgeIntervals);
  ...
  for(auto interval : overlappingIntervals) {
    ...
    if((siTool->ContainsTimestep(m_roadmap_intervals[m_roadmap->GetVID(target)], interval.min+edgeWeight))) {
      waitTime = interval.min;// - _sourceDistance;
      isSafe = true;
      break;
    }
    ...
    for(auto& r : vertexIntervals) {
      if(interval.min <= r.min-edgeWeight && interval.max >= r.min-edgeWeight) {
        waitTime = r.min-edgeWeight-_sourceDistance;
        isSafe = true;
        break;
      }
    }
  ...
  if(!(siTool->ContainsTimestep(edgeIntervals, _sourceDistance+waitTime))) {
    // return std::numeric_limits<double>::infinity();
  ...
  for(auto ei = rit->begin(); ei != rit->end(); ++ei) {
    ...
    if(!m_changed[target]) {
      ...
      for(auto interval : t_intervals) {
        auto intvl = IntervalOverlaps(safe_intervals, interval);
