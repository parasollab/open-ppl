...
MedialAxisLP::
MedialAxisLP(XMLNode& _node) : LocalPlannerMethod(_node),
...
Init() {
  ...
  m_macVC = new MedialAxisClearanceValidity(
      m_medialAxisUtility, m_macEpsilon);

  //Local planner methods
  m_envLP = StraightLine(
      m_medialAxisUtility.GetValidityCheckerLabel(), true);
  m_macLP = StraightLine("MAC::" + this->GetNameAndLabel(), true);
...
bool
MedialAxisLP::
IsConnected(const Cfg& _c1, const Cfg& _c2, Cfg& _col,
            LPOutput* _lpOutput,
...
bool
MedialAxisLP::
IsConnectedRec(const Cfg& _c1, const Cfg& _c2, Cfg& _col,
    LPOutput* _lpOutput,
  ...
  LPOutput maLPOutput, tmpLPOutput;
  int cdCounter = 0, nTicks = 0;
  Cfg mid(this->GetTask()->GetRobot());
  Cfg diff = _c1 - _c2;
...
bool
MedialAxisLP::
EpsilonClosePath(const Cfg& _c1, const Cfg& _c2, Cfg& _mid,
    LPOutput* _lpOutput,
  ...
  LPOutput maLPOutput, tmpLPOutput, testLPOutput;
  auto robot = this->GetTask()->GetRobot();
  Cfg col(robot), tmp(robot);
  int nTicks;
  ...
  ///       anything.
  vector<pair<Cfg,Cfg> > history = m_macVC->GetHistory();
...
bool
MedialAxisLP::
IsConnectedIter(const Cfg& _c1, const Cfg& _c2, Cfg& _col,
                LPOutput* _lpOutput,
                double _posRes, double _oriRes) {
...
bool
MedialAxisLP::
IsConnectedBin(const Cfg& _c1, const Cfg& _c2, Cfg& _col,
    LPOutput* _lpOutput,
    double _posRes, double _oriRes) {
  ...
  typedef map<double, pair<Cfg, LPOutput>> PathMap;
  PathMap path;
  typedef pair<pair<double, Cfg>, pair<double, Cfg> > Segment;
  ...
  while(!segQueue.empty() && iter++ < m_maxIter) {
    ...
    //not close enough, so push to MA and push back resulting segments
      ...
      Cfg mid = (seg.first.second + seg.second.second)/2;
...
void
MedialAxisLP::
RemoveBranches(LPOutput* _lpOutput) {

  if(_lpOutput->m_path.empty())
    return;

  vector<Cfg> newPath, newIntermediates;
  ...
  for(CIT cit = _lpOutput->m_path.begin();
    ...
    typename vector<Cfg>::iterator f =
    ...
    typedef typename vector<Cfg>::reverse_iterator RCIT;
...
ReduceNoise(const Cfg& _c1, const Cfg& _c2,
  ...
  for(auto iter = _lpOutput->m_path.begin() + 1;
    ...
    tempGraph.AddEdge(v1, next, typename MPBaseObject::WeightType("edge", dm->Distance(path.back(), *iter)));
    path.push_back(*iter);
    v1 = next;
  }
  ...
  auto smoother = this->GetMPLibrary()->GetPathModifier(m_pathModifier);
