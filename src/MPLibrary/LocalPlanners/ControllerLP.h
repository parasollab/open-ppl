#ifndef CONTROLLER_LP_H_
#define CONTROLLER_LP_H_
...
class ControllerLP : public LocalPlannerMethod<MPTraits> {
  ...
  public:
    ...
    ControllerLP(const std::string& _vcLabel = "",
        bool _saveIntermediates = false);

    ControllerLP(XMLNode& _node);

    virtual ~ControllerLP() = default;
...
template <typename MPTraits>
ControllerLP<MPTraits>::
ControllerLP(const std::string& _vcLabel, bool _saveIntermediates)
        : LocalPlannerMethod<MPTraits>(_saveIntermediates),
          m_vcLabel(_vcLabel) {
  this->SetName("ControllerLP");
}

template <typename MPTraits>
ControllerLP<MPTraits>::
ControllerLP(XMLNode& _node) : LocalPlannerMethod<MPTraits>(_node) {
  this->SetName("ControllerLP");
...
IsConnectedFunc(
  ...
  const std::string id = this->GetNameAndLabel() + "::IsConnectedFunc()";

  // Get Robot
  auto robot = _c1.GetRobot();

  // Get time resolution
  const double dt = robot->GetMPProblem()->GetEnvironment()->GetTimeRes();
  ...
  double distance = 0;
  const CfgType& prev = _c1;

  for(int i = 1; i < numSteps * 2; ++i) {
    // Get next best control
    /// TODO: Figure out why this is not working
    auto bestControl = robot->GetController()->operator()(prev, _c2, dt);
    auto current = robot->GetSimulationModel()->Execute(bestControl);

    // Add the step distance to the total distance.
    distance += dm->Distance(prev, current);
    ...
    if(_checkCollision) {
      ...
      bool inBounds = current.InBounds(env);
      if(!inBounds || !vc->IsValid(current, id)) {
        _col = current;
    ...
    // Save valid intermediate
    if(_savePath)
      _lpOutput->m_path.push_back(current);
    ...
    if(current == _c2) {
      ...
      // Reset robot multibody back to initial configuration
      // Reset simulation model to inital configuration
      robot->GetSimulationModel()->SetState(_c1);
