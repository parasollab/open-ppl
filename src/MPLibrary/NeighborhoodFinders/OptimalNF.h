...
class OptimalNF : public NeighborhoodFinderMethod<MPTraits> {
  ...
  protected:
    ...
    void SetRadiusParameters(const std::string _exLabel, const size_t _dimension, 
                             const size_t _roadmapSize, const double _cFreeHyperVolume);

    void SetKParameters(const size_t _dimension, const size_t _roadmapSize);
    ...
    std::function<void(GroupRoadmapType*)> m_setGroupParameters;
  ...
  if(nfType == "radius") {
    ...
    m_setGroupParameters = [this, e, exLabel](GroupRoadmapType* _r) {
      ...
      for(auto robot : _r->GetGroup()->GetRobots()) {
        ...
        cFreeHyperVolume *= (cspace->GetVolume()
                             * (vspace ? vspace->GetVolume() : 1));

      }

      this->SetRadiusParameters(exLabel,dimension,_r->Size(), cFreeHyperVolume);
    };
    ...
    // Define the parameter function to set K based on the roadmap size and
    // dimension.
    m_setGroupParameters = [this, e](GroupRoadmapType* _r) {
      double dimension = 0;
      for(auto robot : _r->GetGroup()->GetRobots()) {
        auto cspace = robot->GetCSpace();
        auto vspace = robot->GetVSpace();
...
void
OptimalNF<MPTraits>::
SetRadiusParameters(const std::string _exLabel, const size_t _dimension, 
                    const size_t _roadmapSize, const double _cFreeHyperVolume) {

  const double rd = 1. / _dimension;

  // The actual hypervolume of a unit ball in R^dimension space.
  // Ref: https://en.wikipedia.org/wiki/Volume_of_an_n-ball
  const double halfD = _dimension / 2.;
  const double unitBallHyperVolume = std::pow(PI, halfD)
                                   / std::tgamma(halfD + 1);
  ...
  // (although we've instead used the number of balls in cspace since we
  // can't compute the measure of cfree).
  const double volumeRatio = _cFreeHyperVolume / unitBallHyperVolume;

  // The optimal radius will depend on (lg x / x) where x is the size of the
  // roadmap. This reaches its maximum at x = e, so we'll lower-bound the
  // effective size at e to avoid numerical problems and sillyness with an
  // expanding radius.
  const double size = std::max<double>(std::exp(1), _roadmapSize);

  // Find the optimal radius.
  const double gamma = 2. * std::pow(1. + rd, rd) * std::pow(volumeRatio, rd);
  const double optimalR = gamma * std::pow(std::log(size) / size, rd);

  if(_exLabel.empty()) {
    this->m_nf->GetRadius() = optimalR;
  }
  else {
    auto ex = this->GetExtender(_exLabel);
    this->m_nf->GetRadius() = std::min(optimalR, ex->GetMaxDistance());
  }
...
void
OptimalNF<MPTraits>::
SetKParameters(const size_t _dimension, const size_t _roadmapSize) {
  constexpr double e = std::exp(1.);
  const double k = e * (1. + 1. / _dimension);
  // Force the size to be at least two to get a positive k-value.
  const size_t optimalK = std::ceil(k * std::log(std::max<size_t>(_roadmapSize, 2)));
  this->m_nf->GetK() = optimalK;
  if(this->m_debug)
    std::cout << "Finding closest neighbors with k = " << this->m_nf->GetK()
              << "."
