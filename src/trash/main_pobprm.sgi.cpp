...
(Roadmap * rdmp, CollisionDetection *cd,LocalPlanners* lp, DistanceMetric * dm, 
 ...
 DistanceMetric * dm, CN& _cn, CNInfo& info)
{
        ...
        ConnectMapNodes::ConnectNodes_Closest
        (_rm, cd, lp, dm, _cn, info, cfgs, allVectices, k);
        m_sync(); //barrier
}
...
void pConnectMapNodes::ReplaceConnector(){
        ...
        CNF CM_CLOSEST = (CNF)ConnectMapNodes::ConnectNodes_Closest;
        CNF CM_CC = (CNF)ConnectMapNodes::ConnectNodes_ConnectCCs;
...
Input * CPOBPRM_MPComponents::m_pInput=NULL;
Roadmap * CPOBPRM_MPComponents::m_pMap=NULL;

unsigned int CPOBPRM_MPComponents::m_NumberOfProcessor=0;

bool
CPOBPRM_MPComponents::doInit(){
...
void Init(Roadmap& ,Input & ,CPOBPRM_MPComponents *);
void GenerateMapNodes(Roadmap& ,Input & ,CPOBPRM_MPComponents *);
void ConnectMapNodes(Roadmap& ,Input &, CPOBPRM_MPComponents *);
void OutputMap(Roadmap& ,Input & ,CPOBPRM_MPComponents *);
...
int main( int argc, char ** argv ){
        ...
        m_set_procs(cNumProc);
        CPOBPRM_MPComponents::m_NumberOfProcessor = cNumProc;

        //create components
        CPOBPRM_MPComponents * MPC = new CPOBPRM_MPComponents[cNumProc];
        if( MPC==NULL ){
                cerr<<"! pOBPRM Error: Not Enough Memory"<<endl;
                return 1;
        }
...
void Init(Roadmap& rmap, Input & input, CPOBPRM_MPComponents * MPC){
        ...
        CPOBPRM_MPComponents::SetInput(&input);
        CPOBPRM_MPComponents::SetRoadmap(&rmap);
        
        //init CPOBPRM_MPComponents in Parallel
        //m_fork(p_Init, MPC);
 	// with -64 compiler option, it has stricter requirements.
 	m_fork((void (*)())p_Init, MPC);
}

void GenerateMapNodes(Roadmap& rmap,Input & input, CPOBPRM_MPComponents * MPC )
{
        //read from file
...
void ConnectMapNodes(Roadmap& rmap,Input & input, CPOBPRM_MPComponents * MPC )
{
        //setup clocks
        ...
        //start to connect
        //m_fork(p_CNodes, MPC);
	m_fork((void (*)())p_CNodes, MPC);
        //print out clock
        NodeConClock.StopPrintClock();
        //output info
        cout<< "- pOBPRM Msg : "<<rmap.m_pRoadmap->GetCCcount() 
            << " Connected Components"<<endl;
}

void OutputMap(Roadmap& rmap,Input & input, CPOBPRM_MPComponents * MPC)
{
