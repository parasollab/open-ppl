...
class MedialAxisRRT : public BasicRRTStrategy<MPTraits> {
  public:
    ...
    MedialAxisRRT(MPProblemType* _problem, XMLNode& _node);
...
template<class MPTraits>
MedialAxisRRT<MPTraits>::MedialAxisRRT(MPProblemType* _problem, XMLNode& _node) :
...
template<class MPTraits>
void
MedialAxisRRT<MPTraits>::ParseXML(XMLNode& _node) {
  m_extendDist = _node.Read("extendDist", true, 0.5, 0.0, MAX_DBL, "Step size for regular RRT extend");
  m_maxIntermediates = _node.Read("maxIntermediates", true, 5, 1, MAX_INT, "Maximum Number Of Intermediates");
}
