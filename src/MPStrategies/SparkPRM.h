...
ParseXML(XMLNodeReader& _node) {
  ...
  m_cLabel = _node.stringXMLParameter("cLabel", true, "", "Node Connection Method");
...
ConnectVertex(RoadmapType* _centroidRdmp, vector<VID>& _notRRT, VID _recentVID) {
  ...
  ConnectorPointer connector = this->GetMPProblem()->GetConnector(m_cLabel);
  stapl::sequential::vector_property_map<GraphType, size_t> cMap;
  GraphType* centroidGraph = _centroidRdmp->GetGraph();
    ...
    if(_connectedCCs == 2 || (m_trimAll && _connectedCCs == 1)) {
      ...
      if(this->m_rrtDebug) {
        ...
        for(size_t i = 0; i < path.size(); i++)
          cout << " " << path[i];
        cout << "\ntrimmed:";
        for(typename set<VID>::iterator it = trimmed.begin(); it != trimmed.end(); it++)
          cout << " " << *it;
        cout << "\nDeleted VIDs:";
      }
