...
Run(std::vector<WholeTask*> _wholeTasks, std::shared_ptr<TaskPlan> _plan) {
	...
	this->GetTaskPlan()->GetStatClass()->StartClock("PlanningTime");	
	...
	//size_t explored;
	//double cost;
	CBSSolution solution = ConflictBasedSearch(decomp, init, valid,this->GetTaskPlan()->GetStatClass(),MAX_INT,true);//,total,explored,cost);	
	...
	//this->GetTaskPlan()->GetStatClass()->SetStat("NodesExplored", explored);
	//this->GetTaskPlan()->GetStatClass()->SetStat("SOC", cost);
	this->GetTaskPlan()->GetStatClass()->StopClock("PlanningTime");	

  //Simulation::Get()->PrintStatFile();
	this->GetTaskPlan()->PrintStatFile();
...
*/
	...
	*/
	...
	this->GetTaskPlan()->GetStatClass()->SetStat("TotalNodes", totalNodes);
	this->GetTaskPlan()->GetStatClass()->SetStat("MaxDepth", maxDepth);
	this->GetTaskPlan()->GetStatClass()->SetStat("SolutionDepth", solutionDepth);
	this->GetTaskPlan()->GetStatClass()->SetStat("NodesExplored", nodesExplored);
	this->GetTaskPlan()->GetStatClass()->SetStat("Makespan", minNode->GetDiscreteCost(true));
	this->GetTaskPlan()->GetStatClass()->SetStat("SOC", minNode->GetDiscreteCost(false));
  Simulation::Get()->PrintStatFile();
...
UpdatePlan(Node* _node) {
	...
	for(auto member : this->GetTaskPlan()->GetTeam()) {
		//auto cfg = member->GetRobot()->GetSimulationModel()->GetState();
		auto cfg = this->GetMPProblem()->GetInitialCfg(member->GetRobot());
		int x = int(cfg[0]+.5);
		int y = int(cfg[1]+.5);
		cfg.SetData({double(x),double(y),0});
		cfg.SetRobot(this->GetTaskPlan()->GetCapabilityAgent(member->GetCapability())->GetRobot());
...
CreateDecomposition(std::vector<WholeTask*> _wholeTasks) {
	...
	for(auto wholeTask : _wholeTasks) {
		auto semanticTask = std::shared_ptr<SemanticTask>(new SemanticTask(top.get(),decomp,wholeTask->m_task));
