...
TaskPlan::
TaskPlan(MPProblem* _problem) : m_problem(_problem) {}
...
InitializeRAT(){
  ...
  for(auto agent : m_memberAgents){
    ...
    auto cfg = agent->GetRobot()->GetMPProblem()->GetInitialCfg(agent->GetRobot());
...
PrintStatFile(const std::string& _basename) {
  ...
  const std::string basename = _basename.empty() ? m_problem->GetBaseFilename()
                                                 : _basename,
                    fullname = m_problem->GetPath(basename + "-planner.stat");
  ...
  m_statClass->PrintAllStats(osStat);
}
