...
template <class CFG, class WEIGHT>
void 
OptimalConnection<CFG,WEIGHT>::SetDefault() {
	m_radius = false;
}

template <class CFG, class WEIGHT>
void 
OptimalConnection<CFG, WEIGHT>::PrintUsage(ostream& _os){
	PrintOptions(_os);
}

template <class CFG, class WEIGHT>
void 
OptimalConnection<CFG, WEIGHT>::PrintValues(ostream& _os){
	PrintOptions(_os);
}
...
OptimalConnection<CFG, WEIGHT>::ParseXML (XMLNodeReader& _node) {  
	...
	m_lp = _node.stringXMLParameter("lpMethod", true, "", "Local Planner");
...
CreateCopy() {
	NodeConnectionMethod<CFG,WEIGHT>* copy = new OptimalConnection<CFG,WEIGHT>(*this);
	return copy;
}
...
template<typename OutputIterator>
void 
OptimalConnection<CFG,WEIGHT>::ConnectNodes(Roadmap<CFG, WEIGHT>* _rm, StatClass& _stats, 
		bool _addPartialEdge, bool _addAllEdges, OutputIterator _collision) {
	...
	for (typename vector<VID>::iterator iter2 = _closest.begin(); iter2 != _closest.end(); ++iter2) {
		...
		if(this->GetMPProblem()->GetMPStrategy()->GetLocalPlanners()->GetMethod(m_lp)->
				IsConnected(_rm->GetEnvironment(), _stats, dm,
					...
					col, &lpOutput, this->connectionPosRes, this->connectionOriRes, (!_addAllEdges) )) {  
...
OptimalConnection<CFG, WEIGHT>::FindNeighbors(Roadmap<CFG, WEIGHT>* _rm, CFG _cfg, 
	...
	if (m_radius) {
		...
		int k = ceil( 2*2.71828 * log ( _rm->m_pRoadmap->get_num_vertices() ) ) ;  // Rounding up
