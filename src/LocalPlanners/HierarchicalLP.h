...
template<class MPTraits>
HierarchicalLP<MPTraits>::HierarchicalLP(const vector<string>& _lpLabels) : m_lpLabels(_lpLabels) {
...
template<class MPTraits>
bool
HierarchicalLP<MPTraits>::IsConnected(Environment* _env, StatClass& _stats, DistanceMetricPointer _dm,
    const CfgType& _c1, const CfgType& _c2, CfgType& _col, LPOutput<MPTraits>* _lpOutput,
    double _posRes, double _oriRes, bool _checkCollision,
  ...
  for(vector<string>::iterator it = m_lpLabels.begin() ; it!= m_lpLabels.end(); it++){
   LocalPlannerPointer lpMethod = this->GetMPProblem()->GetLocalPlanner(*it);
    if(lpMethod-> IsConnected(_env, _stats, _dm, _c1, _c2, _col, _lpOutput, _posRes,_oriRes,
